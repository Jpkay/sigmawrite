...
3. In `/admin/reviews/assign`, assign exception, sample, or reference passages to one to three independent reviewers. One favorable review meets the current default; additional reviewers may continue in parallel and after publication.
...
5. After one favorable independent review (or the higher threshold on an explicitly escalated version), resolve passages in `/admin/reviews`, publish the approved passages as they become ready; grow the library toward 60 passages,
