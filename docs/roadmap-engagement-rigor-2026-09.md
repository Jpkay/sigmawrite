...
- [ ] **5.2** Multi-revision loop: up to 3 revisions, one priority per pass ("one thing to fix"), diff shown between drafts; remove the single-revision throw in `reviseSummary`. **M**. *Accept:* evidence per revision, mastery only from the final draft.
