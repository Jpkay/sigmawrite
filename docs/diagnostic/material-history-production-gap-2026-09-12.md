...
- For example, `student/settings/page.tsx` supplies a static heading and description and renders `StudentSettings` without recording that page's content. `student/offline/page.tsx` and other client flows require inspection of their actual content and cached delivery behavior. Route enumeration is not proof of complete capture.
