...
**End-to-end verification:** ✅ G01–G29 are composed through the production workflow [`on-demand-workflow.ts`](../src/lib/generation/on-demand-workflow.ts) and its append-only migration [`0057_on_demand_workflow.sql`](../supabase/migrations/0057_on_demand_workflow.sql). A student request is sanitized and idempotently queued; workers claim it without races; rollout, target resolution, immutable contract creation, reuse-first matching, private candidate generation, passage/question/risk QA, set-level decision, and provisional acceptance are recorded as one traceable run. The completion audit applied all migrations `0001–0057` and the reference seed to an empty PostgreSQL database, ran the versioned benchmark suite at 100% overall and critical pass rates, passed 365 application tests, and produced a successful Next.js production build.
