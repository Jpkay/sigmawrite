...
import {PASSIVE_RECOGNITION,PASSIVE_ANALYSES} from '../src/lib/diagnostic/granular/passive-recognition';
...
if(!evidence)throw Error('Approved passive evidence missing');
for(const [index,row] of PASSIVE_RECOGNITION.entries()){
 const choices=PASSIVE_ANALYSES;
 const checked=await runGates({nodeKey:node.key,strand:node.strand,modality:'reading',learnerMode:'shared',responseType:'mcq',promptFr:`${row.sentence}\n\nChoisis l’analyse correcte de cette phrase.`,instructionsFr:'Choisis une réponse.',choices:choices.map((text,i)=>({text,correct:i===row.answer})),validatorType:'exact',difficulty:50,validatorConfig:{materialExposure:{sentences:[row.sentence],assessed:{sentences:[row.sentence]}},...(row.answer!==0?{negativeExample:{excerptFr:row.sentence,rationaleFr:PASSIVE_ANALYSES[row.answer]}}:{})}},{knownNodeKeys:new Set([node.key]),knownMisconceptionKeys:new Set()});
 if(!checked.item||checked.gates.verdict==='rejected')throw Error(`Rejected passive question ${index}: ${JSON.stringify(checked.gates)}`);
 questionMaterialKeys(checked.item);
 const itemKey=`v3-passive-recognition:${index+1}`;
 const entry:CanonicalDiagnosticBankItem={itemKey,item:checked.item,evidenceKey,evidenceExpectation:evidence.expectation,sectionKey:'grammar',promptFamily:'passive-analysis',difficultyTier:'core',reviewStatus:'needs_human_review',qcGates:{...checked.gates,gate3_ensemble:{agrees:false,agreement:0},verdict:'needs_human_review'}};
 items.push(entry);annotations.push({kind:'evidence',itemKey,itemChecksum:checksum(entry),evidenceTarget:{nodeKey:node.key,evidenceKey},contextKey:`passive-recognition:${index+1}`});
}
...
const content={version:'french-v3-passive-recognition-expansion-v1',status:'draft_requires_review',parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(base,artifact.taxonomy).manifest.checksum,items,annotations};
const path='generated/french-v3-passive-recognition-expansion.json',output=JSON.stringify({...content,checksum:checksum(content)},null,2)+'\n';
if(process.argv.includes('--check')){if(readFileSync(path,'utf8')!==output)throw Error('Stale passive expansion');}else writeFileSync(path,output);
