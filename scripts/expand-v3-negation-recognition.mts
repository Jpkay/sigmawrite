...
import {NEGATION_RECOGNITION,NEGATION_ANALYSES} from '../src/lib/diagnostic/granular/negation-recognition';
...
for(const [index,row] of NEGATION_RECOGNITION.entries()){
 ...
 const itemKey=`v3-negation-recognition:${index+1}`;
 const entry:CanonicalDiagnosticBankItem={itemKey,item:checked.item,evidenceKey,evidenceExpectation:evidence.expectation,sectionKey:'grammar',promptFamily:'simple-negation-analysis',difficultyTier:'core',reviewStatus:'needs_human_review',qcGates:{...checked.gates,gate3_ensemble:{agrees:false,agreement:0},verdict:'needs_human_review'}};
 items.push(entry);annotations.push({kind:'evidence',itemKey,itemChecksum:checksum(entry),evidenceTarget:{nodeKey:node.key,evidenceKey},contextKey:`negation-recognition:${index+1}`});
}
...
const content={version:'french-v3-negation-recognition-expansion-v1',status:'draft_requires_review',parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(base,artifact.taxonomy).manifest.checksum,items,annotations};
const path='generated/french-v3-negation-recognition-expansion.json',output=JSON.stringify({...content,checksum:checksum(content)},null,2)+'\n';
