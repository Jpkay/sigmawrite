...
import {CONJUGATION_FOUNDATION_DRAFTS} from "../src/lib/diagnostic/granular/conjugation-foundation-drafts";
...
for(const draft of CONJUGATION_FOUNDATION_DRAFTS){
 ...
 const key=`v3-conjugation-foundation:${draft.key}`;
 const checked=await runGates({nodeKey:node.key,strand:node.strand,modality:"reading",learnerMode:"shared",responseType:"mcq",promptFr:draft.prompt,instructionsFr:"Choisis une réponse.",choices:[{text:draft.answer,correct:true},...draft.distractors.map(text=>({text,correct:false}))],validatorType:"exact",validatorConfig:{materialExposure:{sentences:[draft.prompt,...draft.distractors,draft.answer],assessed:{sentences:draft.nodeKey==="reconnaitre_present_indicatif"?[draft.answer,...draft.distractors]:[draft.prompt]}}},difficulty:50},{knownNodeKeys:new Set([node.key]),knownMisconceptionKeys:new Set()});
 if(!checked.item||checked.gates.verdict==="rejected")throw Error(`Rejected recognition draft: ${key}`);
 const entry:CanonicalDiagnosticBankItem={itemKey:key,item:checked.item,evidenceKey:evidence.key,evidenceExpectation:evidence.expectation,sectionKey:"conjugation",promptFamily:"recognize-conjugation-foundation",difficultyTier:"core",reviewStatus:"needs_human_review",qcGates:{...checked.gates,gate3_ensemble:{agrees:false,agreement:0},verdict:"needs_human_review"}};
 items.push(entry);annotations.push({kind:"evidence",itemKey:key,itemChecksum:checksum(entry),evidenceTarget:{nodeKey:node.key,evidenceKey:evidence.key},contextKey:`conjugation-foundation:${draft.key}`,reason:draft.reason});
}
...
const content={version:"french-v3-conjugation-foundation-expansion-v1",status:"draft_requires_review",parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(base,artifact.taxonomy).manifest.checksum,items,annotations};
const outputPath="generated/french-v3-conjugation-foundation-expansion.json";
const serialized=JSON.stringify({...content,checksum:checksum(content)},null,2)+"\n";
if(process.argv.includes("--check")){
 if(readFileSync(outputPath,"utf8")!==serialized)throw Error("Stale conjugation-foundation expansion: regenerate before reviewing or assembling");
