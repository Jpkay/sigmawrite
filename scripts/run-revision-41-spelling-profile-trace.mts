...
const output=resolve(process.argv[2]??"docs/diagnostic/revision-41-spelling-trace-2026-09-13.json");
...
const base={assessment:candidate.assessment,activities:candidate.activities,candidateChecksum:candidate.checksum,targets:[
 {skillId:"orthographier_nasale_on_om::writing-controlled-production",expected:"known" as const},
 {skillId:"appliquer_m_devant_m_b_p::writing-controlled-production",expected:"weak" as const},
]};
const report=runProfileTrace({...base,profile:"on_om_known_m_before_mbp_weak"});
if(!report.passed)throw Error(`Unresolved revision-41 contrast: ${JSON.stringify(report.contrast)}`);
const omission=runProfileTrace({...base,profile:"on_om_known_m_before_mbp_weak_omission_counterexample",stopAfterQuestions:8});
if(omission.passed)throw Error("Omitted revision-41 spelling target incorrectly passed");
