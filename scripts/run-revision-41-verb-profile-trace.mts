...
const output=resolve(process.argv[2]??"docs/diagnostic/revision-41-etre-avoir-trace-2026-09-13.json");
...
const report=runProfileTrace({assessment:candidate.assessment,activities:candidate.activities,candidateChecksum:candidate.checksum,
 profile:"present_etre_known_avoir_weak",requireSameBranch:false,targets:[
  {skillId:"produire_present_indicatif::writing-controlled-production::verb:être",expected:"known"},
  {skillId:"produire_present_indicatif::writing-controlled-production::verb:avoir",expected:"weak"},
 ]});
if(!report.passed)throw Error(`Unresolved revision-41 contrast: ${JSON.stringify(report.contrast)}`);
const omission=runProfileTrace({assessment:candidate.assessment,activities:candidate.activities,candidateChecksum:candidate.checksum,
 profile:"present_etre_known_avoir_weak_omission_counterexample",requireSameBranch:false,stopAfterQuestions:3,targets:[
  {skillId:"produire_present_indicatif::writing-controlled-production::verb:être",expected:"known"},
  {skillId:"produire_present_indicatif::writing-controlled-production::verb:avoir",expected:"weak"},
 ]});
if(omission.passed)throw Error("Omitted revision-41 target incorrectly passed");
