...
import {IMPERATIVE_MEANING_DRAFTS} from "../src/lib/diagnostic/granular/imperative-meaning";
...
for(const draft of IMPERATIVE_MEANING_DRAFTS){
 ...
 const key=`v3-imperative-meaning:${draft.key}`;
 ...
 items.push(entry);annotations.push({kind:"evidence",itemKey:key,itemChecksum:checksum(entry),evidenceTarget:{nodeKey:node.key,evidenceKey:evidence.key},contextKey:`imperative-meaning-situation:${draft.key}`,reason:draft.reason});
}
...
const content={version:"french-v3-imperative-meaning-expansion-v1",status:"draft_requires_review",parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(base,artifact.taxonomy).manifest.checksum,items,annotations};
const outputPath="generated/french-v3-imperative-meaning-expansion.json";
