import {ARGUMENT_THESIS_DRAFTS} from "../src/lib/diagnostic/granular/argument-thesis";
...
const artifact=read("generated/french-taxonomy-v3.json"),bank=read("generated/diagnostic-bank-v3-draft.json") as CanonicalDiagnosticBankArtifact;
 const drafts=ARGUMENT_THESIS_DRAFTS;
...
for(const draft of drafts){
 ...
 const key=`v3-argument-thesis:${draft.key}`;
...
const content={version:"french-v3-argument-thesis-expansion-v1",status:"draft_requires_review",parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(bank,artifact.taxonomy).manifest.checksum,items,annotations};
emit("generated/french-v3-argument-thesis-expansion.json",JSON.stringify({...content,checksum:checksum(content)},null,2)+"\n");
const summary={status:content.status,authoredItems:items.length,distinctPassages:annotations.length,targets:[...new Set(annotations.map(a=>a.facetKey))],eligibleAdded:checked.eligibleItemKeys.filter(id=>id.startsWith("v3-argument-thesis:")).length,
 limitations:["Original fictional passages; draft content requires pedagogical review", "Identifying the defended thesis in short arguments; argument evaluation remains separate", "Four answer choices; minimum confirmation must account for guessing without multiplying correlated support-choice probabilities", "Consolidated review assembly is separate; no approval or publication"],questions:drafts.map(d=>({key:d.key,words:d.passage.split(/\s+/).length,reviewReason:d.reason}))};
emit("docs/diagnostic/v3-argument-thesis-expansion.json",JSON.stringify(summary,null,2)+"\n");
