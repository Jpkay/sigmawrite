...
do $$
declare
 parent record; old_member record; new_bank uuid; new_item uuid:='601a0eae-ef70-533e-9963-62b3fd5ee25a';
...
begin
 ...
 values('french-diagnostic-bank-v3','parallel-fixture',parent.taxonomy_release_id,'draft',jsonb_build_object('checksum',digest),digest,'{"valid":true,"mode":"parallel_review"}',parent.published_by,now()) returning id into new_bank;
 ...
 select jsonb_build_object('bank',jsonb_build_object('key','french-diagnostic-bank-v3'),'items',jsonb_build_array(jsonb_build_object(
  'itemKey','parallel-publication-fixture','evidenceKey',(select substring(stable_key from strpos(stable_key,':')+1) from taxonomy_release_memberships where release_id=parent.taxonomy_release_id and record_type='mastery_evidence' and record_id=old_member.mastery_evidence_id),
 ...
 begin
  perform publish_granular_parallel_release('tampered-canonical-answer',jsonb_set(parallel_bundle,'{bank,items,0,item,correctAnswer}','"changed answer"'::jsonb),bundle_digest,proof,parent.published_by);
  raise exception 'Changed canonical answer accepted';
 exception when raise_exception then if sqlerrm<>'Canonical question differs: parallel-publication-fixture' then raise; end if; end;
 begin
  perform publish_granular_parallel_release('tampered-canonical-choices',jsonb_set(parallel_bundle,'{bank,items,0,item,choices}','[{"text":"changed","correct":true}]'::jsonb),bundle_digest,proof,parent.published_by);
  raise exception 'Changed canonical choices accepted';
 exception when raise_exception then if sqlerrm<>'Canonical answer choices differ: parallel-publication-fixture' then raise; end if; end;
 insert into granular_assessment_releases(release_key,taxonomy_release_id,bank_release_id,status,content_checksum,bundle)
 select 'occupied-publication-key',taxonomy_release_id,bank_release_id,'draft',content_checksum,bundle
 from granular_assessment_releases where bank_release_id=parent.id limit 1;
 begin
  perform publish_granular_parallel_release('occupied-publication-key',parallel_bundle,bundle_digest,proof,parent.published_by);
 ...
 published_id:=publish_granular_parallel_release('first-parallel-publication',parallel_bundle,bundle_digest,proof,parent.published_by);
 ...
 begin
  insert into granular_assessment_releases(release_key,taxonomy_release_id,bank_release_id,status,content_checksum,bundle)
  values('wrong-parallel-bundle',parent.taxonomy_release_id,new_bank,'published','sha256:'||repeat('c',64),parallel_bundle);
 ...
 insert into granular_assessment_releases(release_key,taxonomy_release_id,bank_release_id,status,content_checksum,bundle)
 values('permitted-parallel-bundle',parent.taxonomy_release_id,new_bank,'published',bundle_digest,parallel_bundle);
 ...
 published_id:=publish_granular_parallel_release('rpc-parallel-bundle',parallel_bundle,bundle_digest,proof,parent.published_by);
 retry_id:=publish_granular_parallel_release('rpc-parallel-bundle',parallel_bundle,bundle_digest,proof,parent.published_by);
 if published_id<>retry_id then raise exception 'Publication retry created another release'; end if;
 begin
  perform publish_granular_parallel_release('rpc-parallel-bundle',parallel_bundle||'{"changed":true}',bundle_digest,proof,parent.published_by);
