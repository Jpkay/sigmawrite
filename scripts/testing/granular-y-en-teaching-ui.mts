...
try {
  for (const [lessonIndex, lesson] of Y_EN_TEACHING.entries()) {
    ...
    await page.evaluate(({ id, title }) => {
      ...
      view.learningActivities = [{ skillId: "y-en-fixture", activityId: "fixture-lesson", kind: "instruction", action: "learn", titleFr: title, href: "/student/diagnostic", estimatedMinutes: 5, contentId: id }];
      localStorage.setItem("granular-ui-fixture", JSON.stringify(view)); localStorage.setItem("y-en-teaching", id);
    ...
    await page.screenshot({ path: `/tmp/granular-y-en-lesson-${lessonIndex}.png`, fullPage: true });
  ...
  console.log("Five y/en lessons / 20 exercises passed: mobile layout, examples, hints, corrections, unsent draft reload and completion. Browser fixture only; no authenticated backend or mastery claim.");
