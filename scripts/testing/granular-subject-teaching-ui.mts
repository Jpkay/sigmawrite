...
try{
 const page=await browser.newPage({viewport:{width:390,height:844}}),errors:string[]=[],lesson=process.argv[2]?SUBJECT_TEACHING.find(lesson=>lesson.id===process.argv[2])!:SUBJECT_TEACHING[0];
 assert.ok(lesson,"Unknown subject lesson");
 ...
 await page.evaluate(lesson=>{
  localStorage.setItem("subject-teaching",lesson.id);
 ...
 await page.screenshot({path:"/tmp/granular-subject-teaching-mobile.png",fullPage:true});
 ...
 console.log("Subject teaching browser fixture passed: selections, hints, conflict/network retention, reload, completion, mobile width.");
