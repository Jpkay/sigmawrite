...
try {
  for (const [lessonIndex, lesson] of DETERMINER_PRODUCTION_TEACHING.entries()) {
    ...
    await page.evaluate(({ id, title }) => {
      ...
      view.learningActivities = [{ skillId: "determiner-production-fixture", activityId: "fixture-lesson", kind: "instruction", action: "learn", titleFr: title, href: "/student/diagnostic", estimatedMinutes: 5, contentId: id }];
      localStorage.setItem("granular-ui-fixture", JSON.stringify(view)); localStorage.setItem("determiner-agreement-teaching", id);
    ...
    await page.screenshot({ path: `/tmp/granular-determiner-production-lesson-${lessonIndex}.png`, fullPage: true });
    ...
    for (const [index, exercise] of lesson.practice.entries()) {
      ...
      await page.reload(); await page.getByLabel("Ta réponse", { exact: true }).waitFor();
      assert.equal(await page.getByLabel("Ta réponse", { exact: true }).inputValue(),index === 0 ? "réponse incorrecte" : exercise.answerFr);
  ...
  console.log("Determiner production lesson / 8 exercises passed: mobile layout, examples, hints, corrections, unsent draft reload and completion. Browser fixture only; no authenticated backend or mastery claim.");
