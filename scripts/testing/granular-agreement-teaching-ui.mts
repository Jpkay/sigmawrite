...
try {
  for (const [lessonIndex, lesson] of AGREEMENT_TEACHING.entries()) {
    ...
    await page.evaluate(({ id, title }) => {
      ...
      view.learningActivities = [{ skillId: "agreement-fixture", activityId: "fixture-lesson", kind: "instruction", action: "learn", titleFr: title, href: "/student/diagnostic", estimatedMinutes: 5, contentId: id }];
      localStorage.setItem("granular-ui-fixture", JSON.stringify(view)); localStorage.setItem("agreement-teaching", id);
    ...
    await page.screenshot({ path: `/tmp/granular-agreement-lesson-${lessonIndex}.png`, fullPage: true });
  ...
  console.log("Four agreement lessons / 16 exercises passed: mobile layout, examples, hints, corrections, reload and completion. Browser fixture only; no authenticated backend or mastery claim.");
