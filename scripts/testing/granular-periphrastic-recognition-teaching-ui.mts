...
try{
 const page=await browser.newPage({viewport:{width:390,height:844}}),errors:string[]=[],lesson=process.argv[2]?PERIPHRASTIC_RECOGNITION_TEACHING.find(lesson=>lesson.id===process.argv[2])!:PERIPHRASTIC_RECOGNITION_TEACHING[0];
 assert.ok(lesson,"Unknown recognition lesson");
 ...
 await page.evaluate(lesson=>{
  localStorage.setItem("periphrastic-recognition-teaching",lesson.id);
  localStorage.setItem("granular-ui-fixture",JSON.stringify({phase:"learning",paused:true,provisional:true,remainingSeconds:0,pendingItemId:null,results:[],priorities:[],sessionId:"11111111-1111-4111-8111-111111111111",revision:0,answeredCount:3,skippedCount:0,teaching:null,learningCheck:null,missingLearningActivityCount:0,deferredReviewCount:0,skillDetails:{},question:null,
   learningActivities:[{skillId:"subject",activityId:"fixture-lesson",kind:"instruction",action:"learn",titleFr:lesson.titleFr,href:"/student/diagnostic",estimatedMinutes:5,contentId:lesson.id}]}));
 },lesson);
 ...
 await page.screenshot({path:"/tmp/granular-periphrastic-recognition-teaching-mobile.png",fullPage:true});
 ...
 console.log("Periphrastic recognition teaching browser fixture passed: selections, hints, conflict/network retention, reload, completion, mobile width.");
