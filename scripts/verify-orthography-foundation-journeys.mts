...
if([imperativeFamilies,complexNegationOnly,mainIdeaOnly,completiveOnly,subjunctiveOnly,subjunctiveFamilies].filter(Boolean).length>1)throw Error('Select one content family');
const lessons=bundle.teachingContent!.filter(l=>imperativeFamilies?(l.id.startsWith('french-v3-teaching:imperatif-present:pattern:')||l.id==='french-v3-teaching:imperatif:recognition'):complexNegationOnly?l.id==='french-v3-teaching:complex-negation:recognition':mainIdeaOnly?l.nodeKey==='identifier_idee_phrase':subjunctiveFamilies?l.id.startsWith('french-v3-teaching:subjonctif-present:pattern:'):subjunctiveOnly?(l.nodeKey==='reconnaitre_subjonctif_present'||(l.nodeKey==='produire_subjonctif_present_frequent'&&l.facetKey?.includes('::verb:'))):completiveOnly?l.id==='french-v3-teaching:completive:production':['segmenter_syllabes_ecrites','associer_phoneme_graphie_frequente','employer_cedille'].includes(l.nodeKey));
if(lessons.length!==(imperativeFamilies?5:complexNegationOnly?1:mainIdeaOnly?3:subjunctiveFamilies?4:subjunctiveOnly?15:completiveOnly?1:6))throw Error('Unexpected number of selected lessons');
const reports=[];
...
writeFileSync(imperativeFamilies?'docs/diagnostic/imperative-family-learning-journeys-2026-09-12.json':complexNegationOnly?'docs/diagnostic/complex-negation-learning-journey-2026-09-12.json':mainIdeaOnly?'docs/diagnostic/sentence-main-idea-learning-journeys-2026-09-12.json':subjunctiveFamilies?'docs/diagnostic/subjonctif-family-learning-journeys-2026-09-12.json':subjunctiveOnly?'docs/diagnostic/subjonctif-learning-journeys-2026-09-12.json':completiveOnly?'docs/diagnostic/completive-production-learning-journey-2026-09-12.json':'docs/diagnostic/orthography-foundation-learning-journeys-2026-09-12.json',JSON.stringify(output,null,2)+'\n');console.log(JSON.stringify(reports));
