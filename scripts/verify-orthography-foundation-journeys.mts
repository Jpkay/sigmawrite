...
if([etreAgreement,verbFamilies,revision35,revision34,passeSimpleFamilies,imperativeVerbs,imperativeFamilies,complexNegationOnly,mainIdeaOnly,completiveOnly,subjunctiveOnly,subjunctiveFamilies].filter(Boolean).length>1)throw Error('Select one content family');
const lessons=bundle.teachingContent!.filter(l=>etreAgreement?l.nodeKey==='accorder_participe_etre':verbFamilies?l.id.startsWith('french-v3-teaching:verb-family:'):revision35?revision35Ids.has(l.id):revision34?revision34Ids.has(l.id):passeSimpleFamilies?(l.id.startsWith('french-v3-teaching:passe-simple:pattern:')||l.id.startsWith('french-v3-teaching:past-foundation:')):imperativeVerbs?l.id.startsWith('french-v3-teaching:imperatif-present:verb:'):imperativeFamilies?(l.id.startsWith('french-v3-teaching:imperatif-present:pattern:')||l.id==='french-v3-teaching:imperatif:recognition'):complexNegationOnly?l.id==='french-v3-teaching:complex-negation:recognition':mainIdeaOnly?l.nodeKey==='identifier_idee_phrase':subjunctiveFamilies?l.id.startsWith('french-v3-teaching:subjonctif-present:pattern:'):subjunctiveOnly?(l.nodeKey==='reconnaitre_subjonctif_present'||(l.nodeKey==='produire_subjonctif_present_frequent'&&l.facetKey?.includes('::verb:'))):completiveOnly?l.id==='french-v3-teaching:completive:production':['segmenter_syllabes_ecrites','associer_phoneme_graphie_frequente','employer_cedille'].includes(l.nodeKey));
if(lessons.length!==(etreAgreement?3:verbFamilies?3:revision35?10:revision34?7:passeSimpleFamilies?6:imperativeVerbs?12:imperativeFamilies?5:complexNegationOnly?1:mainIdeaOnly?3:subjunctiveFamilies?4:subjunctiveOnly?15:completiveOnly?1:6))throw Error('Unexpected number of selected lessons');
const reports=[];
