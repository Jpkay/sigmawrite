...
const passeRecentModalFamily=process.argv.includes('--passe-recent-modal-family');
const causeRelationFamily=process.argv.includes('--cause-relation-family');
const causalReadingGenres=process.argv.includes('--causal-reading-genres');
const avoirAgreement=process.argv.includes('--avoir-participle-agreement');
const localDefinitionReading=process.argv.includes('--local-definition-reading');
const questionDetailReading=process.argv.includes('--question-detail-reading');
const etreAgreement=process.argv.includes('--etre-participle-agreement');
const verbFamilies=process.argv.includes('--verb-family-recognition');
const revision35=process.argv.includes('--revision-35-additions');
const revision35Ids=new Set(['dire','prendre','voir','venir','partir','sortir','pouvoir','vouloir','savoir','devoir'].map(verb=>'french-v3-teaching:passe-simple:verb:'+verb));
...
if([passeRecentModalFamily,causeRelationFamily,causalReadingGenres,avoirAgreement,localDefinitionReading,questionDetailReading,etreAgreement,verbFamilies,revision35,revision34,passeSimpleFamilies,imperativeVerbs,imperativeFamilies,complexNegationOnly,mainIdeaOnly,completiveOnly,subjunctiveOnly,subjunctiveFamilies].filter(Boolean).length>1)throw Error('Select one content family');
const lessons=bundle.teachingContent!.filter(l=>passeRecentModalFamily?l.id.startsWith('french-v3-teaching:passe-recent:modal:'):causeRelationFamily?l.id.startsWith('french-v3-teaching:cause-relation:'):causalReadingGenres?l.id.startsWith('french-v3-teaching:causal-reading-genres:'):avoirAgreement?l.nodeKey==='accorder_participe_avoir_cod':localDefinitionReading?l.nodeKey==='deduire_mot_definition_locale':questionDetailReading?l.nodeKey==='associer_information_question':etreAgreement?l.nodeKey==='accorder_participe_etre':verbFamilies?l.id.startsWith('french-v3-teaching:verb-family:'):revision35?revision35Ids.has(l.id):revision34?revision34Ids.has(l.id):passeSimpleFamilies?(l.id.startsWith('french-v3-teaching:passe-simple:pattern:')||l.id.startsWith('french-v3-teaching:past-foundation:')):imperativeVerbs?l.id.startsWith('french-v3-teaching:imperatif-present:verb:'):imperativeFamilies?(l.id.startsWith('french-v3-teaching:imperatif-present:pattern:')||l.id==='french-v3-teaching:imperatif:recognition'):complexNegationOnly?l.id==='french-v3-teaching:complex-negation:recognition':mainIdeaOnly?l.nodeKey==='identifier_idee_phrase':subjunctiveFamilies?l.id.startsWith('french-v3-teaching:subjonctif-present:pattern:'):subjunctiveOnly?(l.nodeKey==='reconnaitre_subjonctif_present'||(l.nodeKey==='produire_subjonctif_present_frequent'&&l.facetKey?.includes('::verb:'))):completiveOnly?l.id==='french-v3-teaching:completive:production':['segmenter_syllabes_ecrites','associer_phoneme_graphie_frequente','employer_cedille'].includes(l.nodeKey));
if(lessons.length!==(passeRecentModalFamily?3:causeRelationFamily?2:causalReadingGenres?2:avoirAgreement?3:localDefinitionReading?3:questionDetailReading?3:etreAgreement?3:verbFamilies?3:revision35?10:revision34?7:passeSimpleFamilies?6:imperativeVerbs?12:imperativeFamilies?5:complexNegationOnly?1:mainIdeaOnly?3:subjunctiveFamilies?4:subjunctiveOnly?15:completiveOnly?1:6))throw Error('Unexpected number of selected lessons');
const reports=[];
...
writeFileSync(passeRecentModalFamily?'docs/diagnostic/passe-recent-modal-family-learning-journeys-2026-09-13.json':causeRelationFamily?'docs/diagnostic/cause-relation-family-learning-journeys-2026-09-13.json':causalReadingGenres?'docs/diagnostic/causal-reading-genres-learning-journeys-2026-09-12.json':avoirAgreement?'docs/diagnostic/avoir-participle-agreement-learning-journeys-2026-09-12.json':localDefinitionReading?'docs/diagnostic/local-definition-reading-learning-journeys-2026-09-12.json':questionDetailReading?'docs/diagnostic/question-detail-reading-learning-journeys-2026-09-12.json':etreAgreement?'docs/diagnostic/etre-participle-agreement-learning-journeys-2026-09-12.json':verbFamilies?'docs/diagnostic/verb-family-recognition-learning-journeys-2026-09-12.json':revision35?'docs/diagnostic/revision-35-addition-learning-journeys-2026-09-12.json':revision34?'docs/diagnostic/revision-34-addition-learning-journeys-2026-09-12.json':passeSimpleFamilies?'docs/diagnostic/passe-simple-family-learning-journeys-2026-09-12.json':imperativeVerbs?'docs/diagnostic/imperative-verb-learning-journeys-2026-09-12.json':imperativeFamilies?'docs/diagnostic/imperative-family-learning-journeys-2026-09-12.json':complexNegationOnly?'docs/diagnostic/complex-negation-learning-journey-2026-09-12.json':mainIdeaOnly?'docs/diagnostic/sentence-main-idea-learning-journeys-2026-09-12.json':subjunctiveFamilies?'docs/diagnostic/subjonctif-family-learning-journeys-2026-09-12.json':subjunctiveOnly?'docs/diagnostic/subjonctif-learning-journeys-2026-09-12.json':completiveOnly?'docs/diagnostic/completive-production-learning-journey-2026-09-12.json':'docs/diagnostic/orthography-foundation-learning-journeys-2026-09-12.json',JSON.stringify(output,null,2)+'\n');console.log(JSON.stringify(reports));
