...
const output=resolve(process.argv[2]??"docs/diagnostic/revision-41-grammar-construction-trace-2026-09-13.json");
...
const base={assessment:candidate.assessment,activities:candidate.activities,candidateChecksum:candidate.checksum,
 targets:[
  {skillId:"construction_subordonnee_relative::writing-controlled-production",expected:"known" as const},
  {skillId:"construction_subordonnee_completive::writing-controlled-production",expected:"weak" as const},
 ]};
const report=runProfileTrace({...base,profile:"relative_clause_known_completive_clause_weak"});
if(!report.passed)throw Error(`Unresolved revision-41 contrast: ${JSON.stringify(report.contrast)}`);
const omission=runProfileTrace({...base,profile:"relative_clause_known_completive_clause_weak_omission_counterexample",stopAfterQuestions:3});
if(omission.passed)throw Error("Omitted revision-41 grammar target incorrectly passed");
