...
for(const draft of DIRECT_OBJECT_DRAFTS){
 ...
 const key=`v3-direct-object:${draft.key}`;
 ...
 if(!checked.item||checked.gates.verdict==="rejected")throw Error(`Rejected direct-object draft: ${key}`);
 const entry:CanonicalDiagnosticBankItem={itemKey:key,item:checked.item,evidenceKey:evidence.key,evidenceExpectation:evidence.expectation,sectionKey:"grammar",promptFamily:"identify-direct-object-with-analysis",difficultyTier:"core",reviewStatus:"needs_human_review",qcGates:{...checked.gates,gate3_ensemble:{agrees:false,agreement:0},verdict:"needs_human_review"}};
 items.push(entry);annotations.push({kind:"evidence",itemKey:key,itemChecksum:checksum(entry),evidenceTarget:{nodeKey:node.key,evidenceKey:evidence.key},contextKey:`direct-object-sentence:${draft.key}`,reason:draft.reason});
}
...
if(validation.issues.length||validation.eligibleItemKeys.some(key=>items.some(entry=>entry.itemKey===key)))throw Error("Invalid or promoted direct-object draft expansion");
const content={version:"french-v3-direct-object-expansion-v1",status:"draft_requires_review",parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(base,artifact.taxonomy).manifest.checksum,items,annotations};
const outputPath="generated/french-v3-direct-object-expansion.json";
const serialized=JSON.stringify({...content,checksum:checksum(content)},null,2)+"\n";
if(process.argv.includes("--check")){
 if(readFileSync(outputPath,"utf8")!==serialized)throw Error("Stale direct-object expansion: regenerate before reviewing or assembling");
