...
const output=resolve(process.argv[2]??"docs/diagnostic/revision-41-aller-tense-trace-2026-09-13.json");
...
const report=runProfileTrace({assessment:candidate.assessment,activities:candidate.activities,candidateChecksum:candidate.checksum,
 profile:"aller_present_known_imparfait_weak",targets:[
  {skillId:"produire_present_indicatif::writing-controlled-production::verb:aller",expected:"known"},
  {skillId:"produire_imparfait::writing-controlled-production::verb:aller",expected:"weak"},
 ]});
...
console.log(JSON.stringify({output,profile:report.profile,questions:report.trace.length,activeSeconds:report.activeSeconds,ending:report.ending,nextActivity:report.nextActivity?.activityId,passed:report.passed},null,2));
