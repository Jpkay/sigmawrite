...
for(const draft of TENSE_RECOGNITION_DRAFTS){
 ...
 const key=`v3-tense-recognition:${draft.key}`;
 const checked=await runGates({nodeKey:node.key,strand:node.strand,modality:"reading",learnerMode:"shared",responseType:"mcq",promptFr:draft.prompt,instructionsFr:"Choisis une réponse.",choices:[{text:draft.answer,correct:true},...draft.distractors.map(text=>({text,correct:false}))],validatorType:"exact",validatorConfig:{materialExposure:{sentences:[draft.prompt,...draft.distractors,draft.answer],...(draft.assessedTexts?{assessed:{sentences:draft.assessedTexts}}:{})}},difficulty:50},{knownNodeKeys:new Set([node.key]),knownMisconceptionKeys:new Set()});
 if(!checked.item||checked.gates.verdict==="rejected")throw Error(`Rejected recognition draft: ${key}`);
 const entry:CanonicalDiagnosticBankItem={itemKey:key,item:checked.item,evidenceKey:evidence.key,evidenceExpectation:evidence.expectation,sectionKey:"conjugation",promptFamily:["reconnaitre_auxiliaire","reconnaitre_passe_compose","reconnaitre_plus_que_parfait"].includes(draft.nodeKey)?"recognize-compound-tense":["reconnaitre_imparfait","reconnaitre_futur_simple","reconnaitre_conditionnel_present"].includes(draft.nodeKey)?"recognize-simple-tense":"recognize-periphrastic-tense",difficultyTier:"core",reviewStatus:"needs_human_review",qcGates:{...checked.gates,gate3_ensemble:{agrees:false,agreement:0},verdict:"needs_human_review"}};
 items.push(entry);annotations.push({kind:"evidence",itemKey:key,itemChecksum:checksum(entry),evidenceTarget:{nodeKey:node.key,evidenceKey:evidence.key},contextKey:`tense-sentence:${draft.key}`,reason:draft.reason});
}
...
const content={version:"french-v3-tense-recognition-expansion-v1",status:"draft_requires_review",parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(base,artifact.taxonomy).manifest.checksum,items,annotations};
const outputPath="generated/french-v3-tense-recognition-expansion.json";
const serialized=JSON.stringify({...content,checksum:checksum(content)},null,2)+"\n";
if(process.argv.includes("--check")){
 if(readFileSync(outputPath,"utf8")!==serialized)throw Error("Stale tense-recognition expansion: regenerate before reviewing or assembling");
