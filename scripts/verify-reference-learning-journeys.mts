...
const plusFamilyOnly=process.argv.includes('--plus-que-parfait-families');
const conditionalFamilyOnly=process.argv.includes('--conditionnel-families');
...
if([spellingOnly,futurOnly,recentOnly,imparfaitOnly,composeOnly,futureFamilyOnly,conditionalFamilyOnly,plusFamilyOnly].filter(Boolean).length>1)throw Error('Select one content family');
const spellingIds=['french-v3-teaching:present:pattern:spelling_ger','french-v3-teaching:present:pattern:spelling_cer'];
const lessons=bundle.teachingContent!.filter(l=>plusFamilyOnly?l.id.startsWith('french-v3-teaching:plus-que-parfait:pattern:'):conditionalFamilyOnly?l.id.startsWith('french-v3-teaching:conditionnel-present:pattern:'):futureFamilyOnly?l.id.startsWith('french-v3-teaching:futur-simple:pattern:'):composeOnly?l.id.startsWith('french-v3-teaching:passe-compose:pattern:'):imparfaitOnly?l.id.startsWith('french-v3-teaching:imparfait:pattern:'):recentOnly?l.id.startsWith('french-v3-teaching:passe-recent:production:'):futurOnly?l.id.startsWith("french-v3-teaching:futur-proche:production:"):spellingOnly?spellingIds.includes(l.id):l.id.includes(':reference-foundation:')||l.id==='french-v3-teaching:narrative-demonstrative-reference'||l.id.startsWith('french-v3-teaching:demonstrative-reference:')||l.id.startsWith('french-v3-teaching:lexical-chain:'));
if(lessons.length!==(plusFamilyOnly?4:conditionalFamilyOnly?4:futureFamilyOnly?4:composeOnly?4:imparfaitOnly?4:recentOnly?14:futurOnly?18:spellingOnly?2:10))throw Error('Unexpected selected lesson count');
const reports=[];
...
writeFileSync(plusFamilyOnly?'docs/diagnostic/plus-que-parfait-family-learning-journeys-2026-09-12.json':conditionalFamilyOnly?'docs/diagnostic/conditionnel-family-learning-journeys-2026-09-12.json':futureFamilyOnly?'docs/diagnostic/futur-simple-family-learning-journeys-2026-09-12.json':composeOnly?'docs/diagnostic/passe-compose-family-learning-journeys-2026-09-12.json':imparfaitOnly?'docs/diagnostic/imparfait-family-learning-journeys-2026-09-12.json':recentOnly?'docs/diagnostic/passe-recent-learning-journeys-2026-09-12.json':futurOnly?'docs/diagnostic/futur-proche-learning-journeys-2026-09-11.json':spellingOnly?'docs/diagnostic/present-spelling-learning-journeys-2026-09-11.json':'docs/diagnostic/reference-learning-journeys-2026-09-11.json',JSON.stringify(output,null,2)+'\n');console.log(JSON.stringify(reports));
