...
if([spellingOnly,futurOnly,recentOnly,imparfaitOnly,composeOnly,futureFamilyOnly,conditionalFamilyOnly].filter(Boolean).length>1)throw Error('Select one content family');
const spellingIds=['french-v3-teaching:present:pattern:spelling_ger','french-v3-teaching:present:pattern:spelling_cer'];
const lessons=bundle.teachingContent!.filter(l=>conditionalFamilyOnly?l.id.startsWith('french-v3-teaching:conditionnel-present:pattern:'):futureFamilyOnly?l.id.startsWith('french-v3-teaching:futur-simple:pattern:'):composeOnly?l.id.startsWith('french-v3-teaching:passe-compose:pattern:'):imparfaitOnly?l.id.startsWith('french-v3-teaching:imparfait:pattern:'):recentOnly?l.id.startsWith('french-v3-teaching:passe-recent:production:'):futurOnly?l.id.startsWith("french-v3-teaching:futur-proche:production:"):spellingOnly?spellingIds.includes(l.id):l.id.includes(':reference-foundation:')||l.id==='french-v3-teaching:narrative-demonstrative-reference'||l.id.startsWith('french-v3-teaching:demonstrative-reference:')||l.id.startsWith('french-v3-teaching:lexical-chain:'));
if(lessons.length!==(conditionalFamilyOnly?4:futureFamilyOnly?4:composeOnly?4:imparfaitOnly?4:recentOnly?14:futurOnly?18:spellingOnly?2:10))throw Error('Unexpected selected lesson count');
const reports=[];
