...
// New subjunctive family pathways assess application in sentences. Older isolated
// form drills remain in the canonical bank and in historical published releases.
const sentenceFamilyTargets=new Set(adapted.assessment.probes.filter(p=>p.id.startsWith("v3-subjonctif-family-production:")).map(p=>p.skillId));
