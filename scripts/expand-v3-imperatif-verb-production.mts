...
import {IMPERATIF_VERB_APPLICATIONS} from '../src/lib/diagnostic/granular/imperatif-verb-content';
...
for(const [i,row] of IMPERATIF_VERB_APPLICATIONS.entries()){
 ...
 const itemKey=`v3-imperatif-verb-production:${row.verb}:${row.person}:${i}`;
...
const content={version:'french-v3-imperatif-verb-production-expansion-v1',status:'draft_requires_review',parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(base,artifact.taxonomy).manifest.checksum,items,annotations};
const path='generated/french-v3-imperatif-verb-production-expansion.json',output=JSON.stringify({...content,checksum:checksum(content)},null,2)+'\n';
