...
export function granularBankOptions(args:readonly string[]):{revision?:number;verbFamilyRecognition?:boolean;etreParticipleAgreement?:boolean;questionDetailReading?:boolean;localDefinitionReading?:boolean;avoirParticipleAgreement?:boolean}{
 ...
 return {revision:Number(value),...(familyFlags.length?{verbFamilyRecognition:true}:{}),...(agreementFlags.length?{etreParticipleAgreement:true}:{}),...(readingFlags.length?{questionDetailReading:true}:{}),...(definitionFlags.length?{localDefinitionReading:true}:{}),...(avoirFlags.length?{avoirParticipleAgreement:true}:{})};
}
