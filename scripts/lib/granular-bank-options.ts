...
export function granularBankOptions(args:readonly string[]):{revision?:number;verbFamilyRecognition?:boolean;etreParticipleAgreement?:boolean;questionDetailReading?:boolean;localDefinitionReading?:boolean;avoirParticipleAgreement?:boolean;causalReadingGenres?:boolean;causeRelationFamily?:boolean;passeRecentModalFamily?:boolean}{
 const flags=args.flatMap((arg,index)=>arg==='--bank-revision'?[index]:[]);
 const passeRecentModalFlags=args.filter(arg=>arg==='--passe-recent-modal-family');
 if(passeRecentModalFlags.length>1)throw Error('Duplicate passe recent modal family option');
 ...
 if(passeRecentModalFlags.length&&(Number(value)<43||!causeRelationFlags.length||!causalFlags.length||!avoirFlags.length||!definitionFlags.length||!readingFlags.length||!agreementFlags.length||!familyFlags.length))throw Error('Passe recent modal family requires bank revision 43 or later and preceding refinements');
 return {revision:Number(value),...(familyFlags.length?{verbFamilyRecognition:true}:{}),...(agreementFlags.length?{etreParticipleAgreement:true}:{}),...(readingFlags.length?{questionDetailReading:true}:{}),...(definitionFlags.length?{localDefinitionReading:true}:{}),...(avoirFlags.length?{avoirParticipleAgreement:true}:{}),...(causalFlags.length?{causalReadingGenres:true}:{}),...(causeRelationFlags.length?{causeRelationFamily:true}:{}),...(passeRecentModalFlags.length?{passeRecentModalFamily:true}:{})};
}
