...
for(const draft of PERSON_NUMBER_DRAFTS){
 ...
 const checked=await runGates({nodeKey:node.key,strand:node.strand,modality:"reading",learnerMode:"shared",responseType:"mcq",promptFr:draft.prompt,instructionsFr:"Choisis une réponse.",choices:[{text:draft.answer,correct:true},...draft.distractors.map(text=>({text,correct:false}))],validatorType:"exact",validatorConfig:{personNumberGroup:draft.personNumberGroup,materialExposure:{sentences:[draft.sentence,draft.exerciseSentence],assessed:{sentences:[draft.exerciseSentence]}}},difficulty:50},{knownNodeKeys:new Set([node.key]),knownMisconceptionKeys:new Set()});
