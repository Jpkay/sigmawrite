...
import {IMPERATIF_FAMILY_APPLICATIONS} from '../src/lib/diagnostic/granular/imperatif-family-applications';
...
for(const [i,row] of IMPERATIF_FAMILY_APPLICATIONS.entries()){
 const config={verb:row.verb,tense:'imperatif_present',person:row.person};
 const sentences=[row.sentence,row.sentence.replace('___',row.answer)];
 const raw={nodeKey:node.key,strand:node.strand,modality:'writing',learnerMode:'shared',responseType:'short_answer',promptFr:`${row.audience} Complète avec ${row.verb} à l’impératif présent : ${row.sentence}`,instructionsFr:'Écris seulement le verbe manquant.',correctAnswer:row.answer,acceptableAnswers:[],validatorType:'conjugator',difficulty:50,validatorConfig:{...config,sentenceApplication:row.sentence,finiteResponseSpace:{alternatives:[...new Set((["2s","1p","2p"] as const).map(person=>conjugate(row.verb,"imperatif_present",person)))],rationaleFr:"Le verbe et le temps sont fournis. Les trois formes écrites distinctes (tu, nous et vous) donnent un plancher conservateur de hasard pour cette production contrôlée, pas une rédaction libre."},materialExposure:{words:[{lemma:row.verb,form:row.verb}],sentences,assessed:{sentences}}}};
 ...
 const itemKey=`v3-imperatif-family-production:${row.verb}:${row.person}:${i}`;
 ...
 items.push(entry);annotations.push({itemKey,itemChecksum:checksum(entry),facetKey:`${node.key}::pattern:${row.family}`,contextKey:`imperatif-family-application:${i}`,evidenceFeatures:row.family.startsWith("spelling_")&&row.person==="1p"?["spelling-adjustment"]:[]});
}
...
const content={version:'french-v3-imperatif-family-production-expansion-v1',status:'draft_requires_review',parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(base,artifact.taxonomy).manifest.checksum,items,annotations};
const path='generated/french-v3-imperatif-family-production-expansion.json',output=JSON.stringify({...content,checksum:checksum(content)},null,2)+'\n';
