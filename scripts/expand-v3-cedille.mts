...
import {CEDILLE_DRAFTS} from "../src/lib/diagnostic/granular/cedille-drafts";
...
for(const draft of CEDILLE_DRAFTS){
 const node=artifact.taxonomy.nodes.find((node:{key:string})=>node.key==="employer_cedille"),evidence=node?.evidence.find((evidence:{key:string})=>evidence.key===(draft.mode==="recognition"?"reading-receptive":"writing-controlled-production"));
 if(!evidence)throw Error(`Missing approved target: employer_cedille`);
 const key=`v3-cedille:${draft.key}`;
 const masked=draft.sentence.match(/[\p{L}_]*___[\p{L}_]*/u)![0];
 const wrong=masked.replace("___",draft.letter==="ç"?"c":"ç");
 if(masked.replace("___",draft.letter)!==draft.word)throw Error(`Invalid word mask: ${key}`);
 const reason=draft.letter==="ç"?"La cédille conserve le son s devant a, o ou u.":"Ce mot conserve c sans cédille.";
 ...
 const checked=await runGates({nodeKey:node.key,strand:node.strand,modality:draft.mode==="recognition"?"reading":"writing",learnerMode:"shared",responseType:draft.mode==="recognition"?"mcq":"cloze",promptFr:prompt,instructionsFr:draft.mode==="recognition"?"Choisis une réponse.":"Écris le mot entier.",...(draft.mode==="recognition"?{choices:[{text:draft.word,correct:true},{text:wrong,correct:false}]}:{correctAnswer:draft.word}),validatorType:"exact",validatorConfig:{...(draft.mode==="recognition"?{contrastingErrors:[{errorKey:draft.letter==="ç"?"missing_cedilla":"unneeded_cedilla",incorrectChoiceFr:wrong}]}:{finiteResponseSpace:{alternatives:[draft.word,wrong],rationaleFr:"Le choix ciblé oppose c et ç. La saisie du mot ne supprime pas ce choix binaire."}}),materialExposure:{words:[{lemma:draft.word,form:draft.word}],assessed:{words:[draft.word]}}},difficulty:50},{knownNodeKeys:new Set([node.key]),knownMisconceptionKeys:new Set()});
 if(!checked.item||checked.gates.verdict==="rejected")throw Error(`Rejected spelling draft: ${key}`);
 questionMaterialKeys(checked.item);
 const entry:CanonicalDiagnosticBankItem={itemKey:key,item:checked.item,evidenceKey:evidence.key,evidenceExpectation:evidence.expectation,sectionKey:"spelling",promptFamily:"cedille-in-context",difficultyTier:"core",reviewStatus:"needs_human_review",qcGates:{...checked.gates,gate3_ensemble:{agrees:false,agreement:0},verdict:"needs_human_review"}};
 canonicalProbeMetrics(entry);
 items.push(entry);annotations.push({itemKey:key,itemChecksum:checksum(entry),kind:"evidence",evidenceTarget:{nodeKey:node.key,evidenceKey:evidence.key},contextKey:`cedille:${draft.key}`,reason});
}
...
const content={version:"french-v3-cedille-expansion-v1",status:"draft_requires_review",parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(base,artifact.taxonomy).manifest.checksum,items,annotations};
const outputPath="generated/french-v3-cedille-expansion.json";
const serialized=JSON.stringify({...content,checksum:checksum(content)},null,2)+"\n";
if(process.argv.includes("--check")){
 if(readFileSync(outputPath,"utf8")!==serialized)throw Error("Stale cedille expansion: regenerate before reviewing or assembling");
