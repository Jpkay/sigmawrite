import {LOCAL_DEFINITION_READING_DRAFTS} from "../src/lib/diagnostic/granular/local-definition-reading-drafts";
...
const artifact=read("generated/french-taxonomy-v3.json"),bank=read("generated/diagnostic-bank-v3-draft.json") as CanonicalDiagnosticBankArtifact;
 const drafts=LOCAL_DEFINITION_READING_DRAFTS;
...
for(const draft of drafts){
 ...
 const key=`v3-local-definition-reading:${draft.key}`;
...
const content={version:"french-v3-local-definition-reading-expansion-v1",status:"draft_requires_review",parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(bank,artifact.taxonomy).manifest.checksum,items,annotations};
emit("generated/french-v3-local-definition-reading-expansion.json",JSON.stringify({...content,checksum:checksum(content)},null,2)+"\n");
const summary={status:content.status,authoredItems:items.length,distinctPassages:annotations.length,targets:[...new Set(annotations.map(a=>a.facetKey))],eligibleAdded:checked.eligibleItemKeys.filter(id=>id.startsWith("v3-local-definition-reading:")).length,
 limitations:["Original fictional passages; draft content requires pedagogical review", "Three local-definition genre targets covered by drafts only; inference through examples, morphology and polysemy remain separate", "Four answer choices; minimum confirmation must account for guessing without multiplying correlated support-choice probabilities", "Consolidated review assembly is separate; no approval or publication"],questions:drafts.map(d=>({key:d.key,words:d.passage.split(/\s+/).length,reviewReason:d.reason}))};
emit("docs/diagnostic/v3-local-definition-reading-expansion.json",JSON.stringify(summary,null,2)+"\n");
const packet=`# Questions courtes de lecture : dossier de revue

Statut : brouillons, aucune approbation. ${drafts.length} textes originaux. Les réponses ci-dessous sont destinées à la revue, pas à l’interface élève.

Vérifier le rattachement à la compétence, l’absence d’ambiguïté, la validité des réponses et des indices, la qualité des distracteurs et la difficulté. Plusieurs textes différents ne suffisent pas à établir une difficulté calibrée ou une maîtrise transférable.

${drafts.map(draft=>`## ${draft.key}

Compétence : ${draft.nodeKey}. Genre : ${draft.genre}.

> ${draft.passage}

${draft.question}

- Réponse attendue : ${draft.answer}
${draft.distractors.map(answer=>`- Autre choix : ${answer}`).join("\n")}

Justification textuelle attendue : « ${draft.support} »

Autres extraits proposés :

${draft.otherSpans.map(span=>`- « ${span} »`).join("\n")}

Raison du rattachement : ${draft.reason}
`).join("\n")}`;
emit("docs/diagnostic/v3-local-definition-reading-review-packet.md",packet);
console.log(JSON.stringify({...summary,questions:undefined},null,2));
