...
import {FUTUR_SIMPLE_FAMILY_APPLICATIONS} from '../src/lib/diagnostic/granular/futur-simple-family-applications';
...
const node=artifact.taxonomy.nodes.find((n:{key:string})=>n.key==='produire_futur_simple');
...
for(const [i,row] of FUTUR_SIMPLE_FAMILY_APPLICATIONS.entries()){
 const config={verb:row.verb,tense:'futur_simple',person:row.person};
 const sentences=[row.sentence,row.sentence.replace('___',row.answer)];
 const raw={nodeKey:node.key,strand:node.strand,modality:'writing',learnerMode:'shared',responseType:'short_answer',promptFr:`Complète avec ${row.verb} au futur simple : ${row.sentence}`,instructionsFr:'Écris seulement le verbe manquant.',correctAnswer:row.answer,acceptableAnswers:[],validatorType:'conjugator',difficulty:50,validatorConfig:{...config,sentenceApplication:row.sentence,finiteResponseSpace:{alternatives:[...new Set(PERSONS.map(person=>conjugate(row.verb,"futur_simple",person)))],rationaleFr:"Le verbe et le temps sont fournis. Les six formes écrites distinctes donnent un plancher conservateur de hasard pour cette production contrôlée, pas une rédaction libre."},materialExposure:{words:[{lemma:row.verb,form:row.verb}],sentences,assessed:{sentences}}}};
 const checked=await runGates(raw,{knownNodeKeys:new Set([node.key]),knownMisconceptionKeys:new Set()});if(!checked.item||checked.gates.verdict==='rejected')throw Error(`Rejected futur-simple ${i}`);
 questionMaterialKeys(checked.item);
 const itemKey=`v3-futur-simple-family-production:${row.verb}:${row.person}:${i}`;
 ...
 items.push(entry);annotations.push({itemKey,itemChecksum:checksum(entry),facetKey:conjugationFacet(node.key,config)!,contextKey:`futur-simple-family-application:${i}`});
}
...
const content={version:'french-v3-futur-simple-family-production-expansion-v1',status:'draft_requires_review',parentTaxonomyChecksum:artifact.manifest.contentChecksum,sourceBankChecksum:validateCanonicalDiagnosticBank(base,artifact.taxonomy).manifest.checksum,items,annotations};
const path='generated/french-v3-futur-simple-family-production-expansion.json',output=JSON.stringify({...content,checksum:checksum(content)},null,2)+'\n';
if(process.argv.includes('--check')){if(readFileSync(path,'utf8')!==output)throw Error('Stale futur-simple production');}else writeFileSync(path,output);
