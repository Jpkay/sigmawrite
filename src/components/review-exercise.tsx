...
export type ReviewExerciseProps = {
  ...
  onDecide: (item: CompetencyItemRow, decision: Decision, prompt: string, answer: string, note?: string) => Promise<void>;
};
...
export function ReviewExercise({ item: original, busy, technical = false, onDecide, onLockChange }: ReviewExerciseProps) {
  ...
  function decide(decision: Decision) {
    startTransition(async () => {
      await onDecide(item, decision, item.promptFr, item.correctAnswer ?? "", reporting ? `${reason} — ${note}`.slice(0,1000) : note);
    });
  }
