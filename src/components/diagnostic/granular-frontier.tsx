...
export function GranularFrontier({data,title=copy.title}:{data:GranularFrontierView;title?:string}){
 ...
 return <>
  ...
  <details className="rounded-lg border border-border p-4"><summary className="cursor-pointer font-semibold">{copy.summary.details}</summary>
  {data.coverage&&<p className="mt-5 text-sm text-muted-foreground">{frontierCoverageText(data.coverage.supportedSkillCount,data.coverage.deferredSkillCount)}</p>}
