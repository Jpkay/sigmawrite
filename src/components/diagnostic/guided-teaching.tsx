...
export function GuidedTeaching({teaching,busy,draft,edit,send}:{teaching:NonNullable<AssessmentView["teaching"]>;busy:boolean;draft:string;edit:(value:string)=>void;send:(type:TeachingCommand)=>void}){
 ...
 return <section className="rounded-xl border border-border p-5 sm:p-7">
  {teaching.phase==="lesson"?<>
   ...
   <ol className="space-y-6">{teaching.steps.map((step,index)=><li key={index}><p className="mb-2 whitespace-pre-line font-semibold">{step.exampleFr}</p><p className="text-muted-foreground">{step.explanationFr}</p></li>)}</ol>
  ...
  </>:exercise?<form onSubmit={event=>{event.preventDefault();send("answer_practice");}}>
   ...
   {!exercise.feedback?<>
    ...
    <div className="flex flex-wrap gap-3"><Button type="submit" disabled={busy||!draft.trim()}>{busy?"Enregistrement…":"Vérifier ma réponse"}</Button>
