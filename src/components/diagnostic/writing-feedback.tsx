...
export function WritingFeedbackCard({feedback}:{feedback:WritingFeedback}){
 ...
 return <section aria-labelledby="writing-feedback-title" className="mb-8 rounded-xl border border-border p-5">
  ...
  <p className="mt-3">{mistakes===0?(feedback.checkedCount===1?'Le passage vérifié est correct pour ce point.':`Les ${feedback.checkedCount} passages vérifiés sont corrects pour ce point.`):`${mistakes} passage${mistakes===1?'':'s'} à revoir parmi ${feedback.checkedCount} vérifié${feedback.checkedCount===1?'':'s'}.`}</p>
  <p className="mt-2 text-sm text-muted-foreground">Ce retour porte sur les passages vérifiés pour cette compétence. Tes prochains textes aideront à confirmer tes acquis.</p>
