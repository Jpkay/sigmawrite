 ...
 return <div className="max-w-4xl">
  ...
  {view.teaching?<GuidedTeaching teaching={view.teaching} busy={busy} draft={draft} edit={edit} send={type=>void send(type)}/>:view.phase==="assessing"||checking?<>
   ...
   :question?<form onSubmit={event=>{event.preventDefault();if(question.audio&&audioPlayedKey!==`${view.sessionId}:${question.id}:${question.audio.src}`)return;void send(checking?"answer_check":"answer");}} className="rounded-xl border border-border p-5 sm:p-7">
    <h2 className="mb-4 text-sm font-semibold text-muted-foreground">{checking?copy.newCheck:`Question ${view.answeredCount+(view.skippedCount??0)+1}`}</h2>
   ...
   <section className="mb-8"><h2 className="mb-3 text-xl font-semibold">{copy.firstActivities}</h2><ol className="space-y-3">{(view.learningActivities??[]).map(activity=><li key={activity.activityId} className="rounded-lg border border-border p-4"><p className="font-semibold">{activity.titleFr}</p><p className="my-2 text-sm text-muted-foreground">{activity.action==="verify"?copy.verifyDescription:activity.action==="learn"?copy.learnDescription:copy.consolidateDescription}</p><>{activity.kind==="independent_check"?<Button variant="outline" disabled={busy} onClick={()=>void send("start_check",activity.activityId)}>{copy.verifyPoint}</Button>:activity.contentId?<Button variant="outline" disabled={busy} onClick={()=>void send("start_teaching",activity.activityId)}>{copy.startActivity}</Button>:<Link href={activity.href} className={buttonVariants({variant:"outline"})}>{copy.startActivity}</Link>}</></li>)}</ol>{view.deferredReviewCount>0&&<p className="mt-3 text-sm text-muted-foreground">{copy.deferredHelp}</p>}{!view.learningActivities?.length&&!view.optionalLearningActivities?.length&&(view.deferredReviewCount===0||view.missingLearningActivityCount>0)&&<p className="text-muted-foreground">{copy.activitiesUnavailable}</p>}{Boolean(view.optionalLearningActivities?.length)&&<section className="mt-6"><h3 className="text-lg font-semibold">{copy.optionalTitle}</h3><p className="my-2 text-sm text-muted-foreground">{copy.optionalHelp}</p><ul className="space-y-3">{view.optionalLearningActivities?.map(activity=><li key={activity.activityId} className="rounded-lg border border-border p-4"><p className="mb-3 font-semibold">{activity.titleFr}</p><Button variant="outline" disabled={busy} onClick={()=>void send("start_teaching",activity.activityId)}>{copy.openLesson}</Button></li>)}</ul></section>}<Link href="/student" className={`${buttonVariants()} mt-5`}>{copy.viewPathway}</Link></section>
   ...
   <section><h2 className="mb-4 text-xl font-semibold">{copy.detailsTitle}</h2>{groupAssessmentResults(view.results,view.skillDetails).map(group=><details key={group.id} className="mb-3 rounded-lg border border-border p-4"><summary className="cursor-pointer font-semibold">{group.labelFr}</summary><ul className="mt-4 divide-y divide-border">{group.results.map(({result,detail})=><li key={result.skillId} className="py-3"><p>{detail?.labelFr??copy.unverifiedPoint}</p><p className="text-sm text-muted-foreground">{copy.mode[detail?.mode??result.modes[0]?.mode]??""} · {detail?.assessmentAvailable===false?copy.questionsComing:copy.status[result.status]} · {result.modes.reduce((sum,mode)=>sum+mode.distinctItems,0)} réponse(s)</p><SkillFeatureResults result={result}/></li>)}</ul></details>)}</section>
  </>}
 </div>;
}
