  ...
  return (
      <div className="border-y border-border py-7">
        ...
        {item.responseType === "error_hunt" ? <ErrorHuntWidget sentence={item.promptFr} value={answer} onChange={setAnswer} disabled={disabled} />
          ...
          : item.responseType === "justified" ? <JustifiedWidget choices={item.choices} rules={((item.validatorConfig?.rules as { key: string; label: string }[] | undefined) ?? [])} choice={choice} rule={rule} onChoice={setChoice} onRule={(key) => { setRule(key); setAnswer(key); }} disabled={disabled} />
