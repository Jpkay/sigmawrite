...
import { decidePassageAutomation,enabledPassagePolicyIssues,PASSAGE_QA_VERSION,type AutomatedEvidence,type PassageAutomationPolicyConfig } from './policy';
...
async function fixture(){
 ...
 c.generated.questions=[{questionText:'Où pousse cet arbre ?',questionType:'literal',answerFormat:'multiple_choice',choices:['Dans le jardin.','Dans la maison.','Dans la rue.'],correctAnswer:'Dans le jardin.',skillIds:[],difficulty:20}];
...
describe('selective passage approval',()=>{
 ...
 it('does not accept malformed model provenance',async()=>{const{c,e}=await fixture();e.generatorModel='/';expect(decidePassageAutomation(c,e).reasons).toContain('independent_evaluator_required');});
 ...
 it('requires three distinct choices in the calibrated MCQ scope',async()=>{const{c,e}=await fixture();c.generated.questions[0].choices!.pop();expect(decidePassageAutomation(c,e).reasons).toContain('answer_key_invalid');});
...
describe('enabled passage policy configuration',()=>{
 const valid:PassageAutomationPolicyConfig={enabled:true,pipeline_version:PASSAGE_QA_VERSION,evaluator_model:'openai/gpt-5.4-mini',calibration_id:'calibration',sample_percent:5,cohort_percent:10,exposure_cap:100,reviewer_ids:['reviewer']};
 it('accepts only the calibrated bounded pilot configuration',()=>expect(enabledPassagePolicyIssues(valid,'openai/gpt-5.4-mini')).toEqual([]));
 it('fails closed on model, calibration, sampling, cohort, exposure, or reviewer drift',()=>expect(enabledPassagePolicyIssues({...valid,evaluator_model:'other',calibration_id:null,sample_percent:4,cohort_percent:11,exposure_cap:101,reviewer_ids:['reviewer','reviewer']},'openai/gpt-5.4-mini')).toEqual(expect.arrayContaining(['calibration_required','evaluator_model_mismatch','sample_percent_out_of_bounds','cohort_percent_out_of_bounds','exposure_cap_out_of_bounds','reviewer_configuration_invalid'])));
});
