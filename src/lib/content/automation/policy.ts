...
export type PassageAutomationPolicyConfig = {
  enabled: boolean;
...
export function enabledPassagePolicyIssues(policy: PassageAutomationPolicyConfig, configuredEvaluatorModel: string): string[] {
  if (!policy.enabled) return [];
  const issues: string[] = [];
  if (policy.pipeline_version !== PASSAGE_QA_VERSION) issues.push("pipeline_version_mismatch");
  if (!policy.calibration_id) issues.push("calibration_required");
  if (!policy.evaluator_model || policy.evaluator_model !== configuredEvaluatorModel.trim()) issues.push("evaluator_model_mismatch");
  if (policy.sample_percent < 5 || policy.sample_percent > 100) issues.push("sample_percent_out_of_bounds");
  if (policy.cohort_percent < 1 || policy.cohort_percent > 10) issues.push("cohort_percent_out_of_bounds");
  if (policy.exposure_cap < 1 || policy.exposure_cap > 100) issues.push("exposure_cap_out_of_bounds");
  if (policy.reviewer_ids.length < 1 || policy.reviewer_ids.length > 3 || new Set(policy.reviewer_ids).size !== policy.reviewer_ids.length) issues.push("reviewer_configuration_invalid");
  return issues;
}
...
export function decidePassageAutomation(candidate: ContentCandidate, evidence: AutomatedEvidence, samplePercent = 5): PassageDecision {
  ...
  const normalizedModel = (model: string) => model.toLowerCase().split("/").at(-1)?.replace(/:.*$/, "").trim() ?? "";
  const generatorModel = normalizedModel(evidence.generatorModel);
  const evaluatorModel = normalizedModel(evidence.evaluatorModel);
  if (!generatorModel || !evaluatorModel || generatorModel === evaluatorModel) reasons.push("independent_evaluator_required");
  ...
  for (const q of text.questions) if (q.answerFormat === "multiple_choice" && (!q.choices || q.choices.length < 3 || new Set(q.choices.map(c => c.trim().toLowerCase())).size !== q.choices.length || q.choices.filter(c => c === q.correctAnswer).length !== 1)) reasons.push("answer_key_invalid");
