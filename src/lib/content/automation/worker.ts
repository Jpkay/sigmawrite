...
import { evaluateAutomatedPassage, evaluatorConfig } from "./evaluate";
import { enabledPassagePolicyIssues, PASSAGE_QA_VERSION, type PassageAutomationPolicyConfig } from "./policy";
...
export async function processPassageAutomation(db: SupabaseClient, limit = 3) {
  ...
  let configuredEvaluatorModel = "";
  try {
    configuredEvaluatorModel = evaluatorConfig().model;
    if (!process.env.LANGUAGETOOL_URL || !process.env.LANGUAGETOOL_API_KEY) throw new Error("Grammar checker unavailable");
  } catch {
    if (policy.enabled) throw new Error("Enabled passage automation is missing its private QA configuration");
    return {mode:"unconfigured",results:[]};
  }
  const configurationIssues = enabledPassagePolicyIssues(policy as PassageAutomationPolicyConfig, configuredEvaluatorModel);
  if (configurationIssues.length) throw new Error(`Enabled passage automation policy is invalid: ${configurationIssues.join(",")}`);
