...
export async function assessReadingIdeas(answer: string, spec: ValidationSpec, judge: ReadingJudge = judgeReading): Promise<ValidationResult> {
  try {
    ...
    if (!missing.length && result.contradiction.present) return { pass: false, validator: "exact", reason: `Vérifie ce passage de ta réponse : « ${result.contradiction.evidence} ». Il contredit le texte ou va au-delà de ce qu’il permet d’affirmer.` };
