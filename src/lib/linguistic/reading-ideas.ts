...
export class ReadingAssessmentError extends Error {
  constructor(cause: unknown) {
    super(READING_RETRY_MESSAGE, { cause });
    this.name = "ReadingAssessmentError";
  }
}

/** Only a complete, explicit admission of not knowing; never a partial answer. */
export function isExplicitNonAnswer(answer: string): boolean {
  const text = answer.normalize("NFC").trim().toLocaleLowerCase("fr")
    .replace(/’/g, "'").replace(/\s+/g, " ").replace(/[.!?…]+$/u, "").trim();
  return /^(?:je (?:ne )?sais pas|je (?:ne )?sais pas répondre|je n'ai (?:aucune idée|pas la réponse)|aucune idée)$/u.test(text);
}
...
export async function assessReadingIdeas(answer: string, spec: ValidationSpec, judge: ReadingJudge = judgeReading): Promise<ValidationResult> {
  try {
    ...
    if (isExplicitNonAnswer(answer)) return { pass: false, validator: "exact", reason: "Cette réponse indique que la compétence reste à travailler." };
    ...
    throw new ReadingAssessmentError(cause);
  }
}
