...
it('teaches ten distinct approved verb targets with each imperative person represented',()=>{
 ...
 expect(IMPERATIF_VERB_TEACHING.map(l=>l.facetKey)).toEqual(['être','avoir','aller','faire','prendre','venir','partir','sortir','dire','voir'].map(v=>`produire_imperatif::verb:${v}`));
 ...
 expect(IMPERATIF_VERB_CONTENT.map(c=>c.forms)).toEqual([['sois','soyons','soyez'],['aie','ayons','ayez'],['va','allons','allez'],['fais','faisons','faites'],['prends','prenons','prenez'],['viens','venons','venez'],['pars','partons','partez'],['sors','sortons','sortez'],['dis','disons','dites'],['vois','voyons','voyez']]);
});
it('keeps all 120 sentence checks distinct from guided material and balances persons',()=>{
 const artifact=JSON.parse(readFileSync('generated/french-v3-imperatif-verb-production-expansion.json','utf8'));
 expect(artifact.items).toHaveLength(120);
 const taught=new Set(IMPERATIF_VERB_TEACHING.flatMap(teachingMaterialKeys)),seen=new Set<string>();
 for(const verb of ['être','avoir','aller','faire','prendre','venir','partir','sortir','dire','voir'])for(const person of ['2s','1p','2p'])expect(IMPERATIF_VERB_APPLICATIONS.filter(r=>r.verb===verb&&r.person===person)).toHaveLength(4);
...
it('uses fresh sentence checks in both phases instead of former isolated-form probes',()=>{
 const candidate=JSON.parse(readFileSync('docs/diagnostic/v3-parallel-review-candidate.json','utf8'));
 for(const verb of ['être','avoir','aller','faire','prendre','venir','partir','sortir','dire','voir']){
