...
import {PERIPHRASTIC_MEANING_DRAFTS as drafts,PERIPHRASTIC_MEANING_TEACHING as lessons} from './periphrastic-meaning-foundations';
...
it('keeps balanced meaning contexts separate from worked examples and guided practice',()=>{
 ...
 const artifact=JSON.parse(readFileSync('generated/french-v3-periphrastic-meaning-foundations-expansion.json','utf8'));
...
it('retains complete supporting pathways for the newly unblocked future-proche writing target',()=>{
 const candidate=JSON.parse(readFileSync('docs/diagnostic/v3-scoped-review-candidate.json','utf8'));
 for(const node of ['employer_futur_proche_en_contexte']){
