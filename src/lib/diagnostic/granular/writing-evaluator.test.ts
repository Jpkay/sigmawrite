...
it("binds evaluator provenance to the protocol and exact task rubric",async()=>{
 ...
 expect(first.evaluator).toMatchObject({version:"french-writing-evaluator-v11",model:"injected-judge",protocolChecksum:expect.stringMatching(/^sha256:/),rubricChecksum:expect.stringMatching(/^sha256:/)});
