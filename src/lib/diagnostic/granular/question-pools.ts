...
const allocatedSchema=z.object({
 ...
 probes:z.array(z.object({id:z.string().min(1),skillId:z.string().min(1),mode:modeSchema,contextId:z.string().min(1),difficulty:z.number().min(0).max(1),expectedSeconds:z.number().positive(),guessProbability:z.number().positive().max(1),usage:z.enum(["initial","learning"]),assessedMaterialKeys:z.array(z.string().regex(/^(word|sentence):sha256:[a-f0-9]{64}$/)).optional(),materialKeys:z.array(z.string().regex(/^(word|sentence):sha256:[a-f0-9]{64}$/)).optional(),evidenceFeatures:z.array(z.string().min(1)).optional()}).passthrough()),
}).passthrough();
