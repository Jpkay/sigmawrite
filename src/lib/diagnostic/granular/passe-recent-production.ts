...
export const PASSE_RECENT_PRODUCTION_TEACHING:readonly TargetTeachingContent[]=CONJUGATION_TEACHING_CASES.filter(model=>model.key.startsWith('pattern:')||PASSE_RECENT_ACTION_VERBS.has(model.model)||Object.hasOwn(PASSE_RECENT_STATE_GUIDED,model.model)).map(model=>{
 ...
 const practice=cases.map((row,i)=>{
  ...
  return {id:`passe-recent:${model.key}:guided-${i}`,promptFr:`Complète avec ${row.verb} au passé récent. Écris le groupe verbal manquant : ${sentence}`,answerFr,hintFr:`Conjugue venir au présent avec le sujet, puis écris de ou d’ et ${row.verb} à l’infinitif.`,explanationFr:`${sentence.replace('___',answerFr)} ${model.model==='pouvoir'?'Pouvoir présente ici une possibilité toute récente.':'L’action est présentée comme terminée depuis peu.'} ${row.verb} reste à l’infinitif.`};
 });
 const first=cases[0],answer=conjugate(first.verb,'passe_recent',first.person);
 const steps:TargetTeachingContent['steps']=[
  {exampleFr:conjugationSentenceGap(recentActionSentence(first.sentence),answer).replace('___',answer),explanationFr:`${model.model==='pouvoir'?'La possibilité est toute récente : la connexion fonctionne enfin.':'L’action est déjà terminée, mais elle est encore proche du moment où l’on parle.'} ${answer} utilise venir au présent, suivi de de et d’un infinitif. Cette construction s’appelle le passé récent.`},
  {exampleFr:PERSONS.map(person=>`${labels[person]} : ${conjugate(model.model,'passe_recent',person)}`).join('\n'),explanationFr:'Choisis viens, vient, venons, venez ou viennent selon qui fait l’action. Le mot après de ne change pas. On écrit je viens de finir, et non je viens de finis ou je viens de fini.'},
  ...(model.model==='pouvoir'?[{exampleFr:'Je peux télécharger le document. Je viens de pouvoir télécharger le document.',explanationFr:'La première phrase dit que le téléchargement est possible. La seconde le présente comme possible tout récemment, par exemple après le retour de la connexion. Dans viens de pouvoir, pouvoir reste à l’infinitif ; télécharger est déjà fourni dans la phrase.'}]:[]),
  ...
  {exampleFr:'Nous venons de finir. Nous venons d’avancer.',explanationFr:'On écrit d’ quand le mot suivant commence par une voyelle : nous venons d’avancer. Il faut toujours garder de ou d’ : nous venons finir ne veut pas dire la même chose que nous venons de finir.'},
 ];
 return {id:`french-v3-teaching:passe-recent:production:${model.key}`,nodeKey:'produire_passe_recent',facetKey:`produire_passe_recent::${model.key}`,mode:'production',status:'draft_requires_review',titleFr:family?`Écrire les verbes comme ${model.model} au passé récent`:`Écrire ${model.model} au passé récent`,learnerQuestionFr:'Comment dire qu’une action vient de se terminer ?',steps,practice,takeawayFr:'Pour parler d’une action qui vient de se terminer, choisis d’abord viens, vient, venons, venez ou viennent selon qui fait l’action. Ajoute de ou d’, puis le mot qui vient après sans le changer : je viens de finir.',boundaryFr:'Ici, on te dit comment parler du passé. Tu ne choisis pas encore toi-même entre « je viens de finir » et « j’ai fini » dans un texte. Attention : dans « je viens au club », viens indique un déplacement.',materialExposure:{words:[...new Set(['venir',model.model,'finir','avancer',...model.cases.map(row=>row.verb)])].map(lemma=>({lemma,form:lemma})),sentences:[...steps.flatMap(step=>step.exampleFr.split('\n')),...practice.map(p=>p.explanationFr.split(model.model==='pouvoir'?' Pouvoir':' L’action')[0])]}};
});
