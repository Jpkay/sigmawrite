...
export const PASSE_RECENT_APPLICATIONS=contexts.map(([verb,person,sentence])=>({verb,person,sentence,answer:conjugate(verb,'passe_recent',person)}));
const labels={'1s':'je','2s':'tu','3s':'il / elle','1p':'nous','2p':'vous','3p':'ils / elles'};
export const PASSE_RECENT_PRODUCTION_TEACHING:readonly TargetTeachingContent[]=CONJUGATION_TEACHING_CASES.filter(model=>model.key.startsWith('pattern:')).map(model=>{
 const practice=model.cases.map((row,i)=>{
  const answerFr=conjugate(row.verb,'passe_recent',row.person),sentence=conjugationSentenceGap(row.sentence,answerFr);
 ...
 const steps:TargetTeachingContent['steps']=[
  {exampleFr:conjugationSentenceGap(first.sentence,answer).replace('___',answer),explanationFr:`L’action est déjà terminée, mais elle est encore proche du moment où l’on parle. ${answer} utilise venir au présent, suivi de de et d’un infinitif. Cette construction s’appelle le passé récent.`},
