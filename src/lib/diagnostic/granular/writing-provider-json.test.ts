...
it('replays captured provider envelopes without changing any grading payload value',async()=>{
 ...
 for(const {source,rawResponse,expectedPayload} of fixture.cases){
...
it('rejects captured payload corruption instead of guessing a grading property',()=>{
 const fixture=JSON.parse(readFileSync('docs/diagnostic/writing/json-envelope-regressions.json','utf8'));
 for(const c of fixture.rejectedCases)expect(()=>parseWritingProviderJson(c.rawResponse)).toThrow();
});
