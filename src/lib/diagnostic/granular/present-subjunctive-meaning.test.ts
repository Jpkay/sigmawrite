...
import {PRESENT_SUBJUNCTIVE_MEANING_DRAFTS as drafts,PRESENT_SUBJUNCTIVE_MEANING_TEACHING as lessons} from './present-subjunctive-meaning';
...
it('keeps balanced meaning contexts separate from worked examples and guided practice',()=>{
 ...
 const artifact=JSON.parse(readFileSync('generated/french-v3-present-subjunctive-meaning-expansion.json','utf8'));
 const seen=new Set<string>();
 expect(artifact.items).toHaveLength(30);
 ...
 for(const lesson of lessons){
  ...
  expect(nodeDrafts).toHaveLength(lesson.nodeKey==='interpreter_usages_present'?12:18);
  for(const category of new Set(nodeDrafts.map(d=>d.category)))expect(nodeDrafts.filter(d=>d.category===category)).toHaveLength(lesson.nodeKey==='interpreter_usages_present'?4:3);
  expect(lesson.practice).toHaveLength(lesson.nodeKey==='interpreter_usages_present'?6:12);
 }
 expect(JSON.stringify(lessons)).not.toMatch(/—|delve into/);
});
it('retains complete supporting pathways for the two newly unblocked writing targets',()=>{
 const candidate=JSON.parse(readFileSync('docs/diagnostic/v3-scoped-review-candidate.json','utf8'));
 for(const node of ['employer_present_indicatif_en_contexte','employer_subjonctif_present_en_contexte']){
