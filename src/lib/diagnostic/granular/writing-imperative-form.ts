...
const supported=new Set(['mettre','attendre','rejoindre',...CONJUGATION_TEACHING_CASES.flatMap(f=>[f.model,...f.cases.map(c=>c.verb)])]);
...
export function checkWritingImperativeForm(input:{infinitive:string;form:string;suffix:string}):{valid:boolean;forms:string[];liaison:boolean}{
 ...
 if(!supported.has(infinitive)||infinitive==='pouvoir')throw Error('Imperative verb outside verified conjugation support');
