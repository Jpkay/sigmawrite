...
export function bindAssessmentRelease(assessment:V3Assessment,ids:{taxonomyId:string;bankId:string}):Release{
 ...
 const sampling=assessment.skills.some(skill=>skill.samplingGroup!==undefined||skill.challengeOrder!==undefined||skill.formFamily!==undefined)
  ?{sampling:assessment.skills.map(skill=>({id:skill.id,domain:skill.domain??skill.branch,group:skill.samplingGroup??skill.domain??skill.branch,branch:skill.branch,...(skill.formFamily===undefined?{}:{formFamily:skill.formFamily}),...(skill.challengeOrder===undefined?{}:{challengeOrder:skill.challengeOrder})})).sort((a,b)=>a.id.localeCompare(b.id))}:{};
 ...
 return {taxonomyId:ids.taxonomyId,bankId:ids.bankId,checksum:checksum({taxonomy:assessment.taxonomyChecksum,bank:assessment.bankChecksum,facets:assessment.facetChecksum??null,...(scope?{releaseScope:checksum(scope)}:{}),...(assessment.reviewPolicy?{reviewPolicy:checksum(assessment.reviewPolicy)}:{}),...(assessment.routeCoveragePolicy?{routeCoveragePolicy:checksum(assessment.routeCoveragePolicy)}:{}),...(assessment.poolChecksum?{pools:assessment.poolChecksum}:{}),...sampling})};
}
