...
export async function getPracticeLanguageCoaching(input: unknown): Promise<CoachingResult> {
  ...
  async function deliver(result: CoachingResult) {
    await journalStudentPayload(studentId, "legacy:practice-language-tip", {
      ...result,
      attemptId: ownedAttemptId,
      display: practiceLanguageCoachingDisplay({ result, requested: data.requested }),
    });
    return result;
  }
  ...
  if (!attempt?.is_correct || attempt.context !== "practice" || !attempt.answer_text) return deliver({ tip: null, available: true });
  const { data: item } = await db.from("competency_items").select("prompt_fr,validator_config").eq("id", attempt.item_id).single();
  if (!item?.validator_config?.readingRubric) return deliver({ tip: null, available: true });
  ...
  if (!data.requested) {
    ...
    if (ordinalError || !shouldOfferLanguageTip(Number(ordinal))) return deliver({ tip: null, available: true });
  }
