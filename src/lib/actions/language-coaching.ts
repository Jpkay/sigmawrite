...
export async function getPracticeLanguageCoaching(input: unknown): Promise<CoachingResult> {
  ...
  const { data: owned, error } = await userDb.from("competency_attempts").select("id,student_id").eq("id", data.attemptId).single();
  if (error || !owned) throw new Error("Réponse introuvable.");
  const db = createServiceClient();
  const { data: attempt } = await db.from("competency_attempts").select("id,student_id,item_id,answer_text,is_correct,attempted_at,context").eq("id", owned.id).single();
  ...
  if (cache?.[column]) return cache[column] as CoachingResult;
  if (!data.requested) {
