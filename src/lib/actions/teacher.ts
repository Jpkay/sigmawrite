...
import { classInviteInputSchema } from "@/lib/invite-config";
...
export async function inviteStudents(input: unknown) {
  ...
  const { data: result, error } = await supabase.rpc("rotate_class_join_code", {
    p_class_id: data.classId,
    p_expires_in_days: data.expiresInDays,
    p_max_uses: data.maxUses,
  });
  if (error) {
    const reason = `${error.code ?? ""} ${error.message ?? ""}`.toLowerCase();
    if (reason.includes("42501") || reason.includes("forbidden")) {
      throw new Error("Vous n’avez pas l’autorisation de gérer les invitations de cette classe.");
    }
    if (reason.includes("class_not_found")) throw new Error("Cette classe n’existe plus ou n’est plus accessible.");
    throw new Error("Le code n’a pas pu être créé. Réessayez dans quelques minutes.");
  }
  const created = Array.isArray(result) ? result[0] : result;
  if (!created) throw new Error("Le code n’a pas pu être créé. Réessayez dans quelques minutes.");
  const expiresAt = created.expires_at as string;
