...
import {practiceLanguageCoachingDisplay} from '@/lib/diagnostic/granular/practice-player-display';
import type {CoachingResult} from '@/lib/content/language-coaching';
const attemptId='11111111-1111-4111-8111-111111111111';
const result:CoachingResult={tip:{kind:'grammar',before:'les cheval',after:'les chevaux',explanationFr:'Au pluriel, on écrit « chevaux ».',recurring:false},available:true};
...
it.each([true,false])('records generated requested=%s tips with the saved attempt and owner',async requested=>{
 ...
 expect(f.journal).toHaveBeenCalledWith('owner','legacy:practice-language-tip',{attemptId,...result,display:practiceLanguageCoachingDisplay({result,requested})});
...
it('records cached tips again at delivery without regenerating or grading',async()=>{
 ...
 expect(f.journal).toHaveBeenCalledWith('owner','legacy:practice-language-tip',{attemptId,...result,display:practiceLanguageCoachingDisplay({result,requested:true})});expect(f.coach).not.toHaveBeenCalled();expect(f.upsert).not.toHaveBeenCalled();
});
...
it('records the exact requested no-change sentence before returning it',async()=>{
 f.coach.mockResolvedValue({tip:null,available:true});
 expect(await getPracticeLanguageCoaching({attemptId,requested:true})).toEqual({tip:null,available:true});
 expect(f.journal).toHaveBeenCalledWith('owner','legacy:practice-language-tip',expect.objectContaining({
  display:expect.objectContaining({status:'Ta formulation convient : rien d’utile à changer ici.'}),
 }));
});
