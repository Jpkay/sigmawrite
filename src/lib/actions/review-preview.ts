...
export async function checkReviewPreview(input: unknown) {
  ...
  const { data: item, error } = await db.from("competency_items").select("id,response_type,validator_type,validator_config,correct_answer,acceptable_answers,competency_item_choices(id,is_correct,feedback_fr)").eq("id", data.id).single();
