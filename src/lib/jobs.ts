...
export async function runFrenchAutomationMonitoring(db:SupabaseClient){
  const [{data:attempts,error},{data:edges},{data:estimates},{data:policy}]=await Promise.all([db.from("competency_attempts").select("student_id,item_id,node_id,is_correct").not("item_id","is",null),db.from("competency_edges").select("id,source_node_id,target_node_id").eq("edge_type","prerequisite"),db.from("student_competency_estimates").select("student_id,node_id,mastery_probability"),db.from("sparse_review_policies").select("id,version,low_risk_sample_percent,medium_risk_sample_percent,always_review_high_risk").eq("active",true).order("version",{ascending:false}).limit(1).maybeSingle()]);if(error)throw new Error(error.message);if(!policy)throw new Error("Sparse review policy missing");
