...
export default async function SupervisorHome() {
  ...
  return <>
    ...
    {summary.length ? <div className="divide-y divide-border border-y border-border">{summary.map((student) => { const source = students.find((row) => row.id === student.id); const gradeLabel = studentSchoolGradeLabel(source?.snap.grade, source?.snap.frenchBackground, language); return <Link key={student.id} href={`/supervisor/students/${student.id}`} className="group flex flex-wrap items-center justify-between gap-3 px-2 py-5 text-foreground hover:bg-muted/30 hover:text-foreground sm:px-4"><div><p className="font-medium">{student.name}</p>{gradeLabel && <p className="mt-1 text-xs text-muted-foreground">{gradeLabel}</p>}<div className="mt-1 flex flex-wrap items-center gap-2"><Badge>{student.band}</Badge>{student.lowEngagement && <Badge variant="secondary">{language === "en" ? "Low engagement" : "Faible engagement"}</Badge>}<span className="text-sm text-muted-foreground">{student.avgSuccess == null ? "—" : `${Math.round(student.avgSuccess * 100)}%`}</span></div></div><span className="grid size-9 place-items-center rounded-full border border-border text-muted-foreground transition-colors group-hover:border-primary group-hover:bg-primary group-hover:text-primary-foreground"><ArrowRight className="size-4" /></span></Link>; })}</div> : <p className="text-sm text-muted-foreground">{language === "en" ? "No student is visible in your current scope. Ask an administrator to assign a school, class, or student." : "Aucun élève n’est visible dans votre périmètre. Demandez à un administrateur d’attribuer une école, une classe ou un élève."}</p>}
  </>;
}
