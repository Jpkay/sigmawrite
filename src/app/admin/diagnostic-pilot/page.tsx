...
export default async function DiagnosticPilotPage() {
  ...
  const [{ data: setting, error: settingError }, { data: students, error: studentError }, { data: enrollments, error: enrollmentError }] = await Promise.all([
    ...
    db.from("diagnostic_pilot_enrollments").select("id,student_id,active,expires_at,note,cohort_kind,feedback_agreement_source,created_at,students(display_name,current_grade)").eq("active", true).gt("expires_at", new Date().toISOString()).order("created_at", { ascending: false }).limit(100),
  ]);
  ...
  return <><PageHeader title="Pilote de feedback" description="Accès temporaire et isolé au diagnostic v2 pendant que la revue éditoriale continue." /><DiagnosticPilotManager enabled={Boolean(setting?.enabled)} students={(students ?? []).map((student) => ({ id: student.id as string, name: student.display_name as string, grade: Number(student.current_grade) }))} enrollments={(enrollments ?? []).map((row) => { const student = row.students as unknown as { display_name: string; current_grade: number } | null; return { id: row.id as string, studentId: row.student_id as string, studentName: student?.display_name ?? "Élève", grade: Number(student?.current_grade ?? 0), active: Boolean(row.active), expiresAt: row.expires_at as string, note: row.note as string | null, cohortKind: row.cohort_kind as "internal_test" | "feedback_participant", agreementSource: row.feedback_agreement_source as "student" | "guardian" | null }; })} /></>;
}
