...
export default async function ItemBankPage({ searchParams }: { searchParams: Promise<{ status?: string; node?: string }> }) {
  ...
  return <>
    ...
    <form className="mb-5 grid gap-3 rounded-md border border-border p-4 sm:grid-cols-3">
      <label className="text-sm">Statut<select name="status" defaultValue={filters.status ?? ""} className="mt-1 h-9 w-full appearance-none rounded-md border border-input bg-background px-3 pr-10"><option className="bg-zinc-950" value="">Tous</option>{["draft","auto_approved","needs_human_review","human_approved","rejected","retired"].map((status) => <option className="bg-zinc-950" key={status}>{status}</option>)}</select></label>
    ...
    <div className="space-y-2">{items.map((item) => <Card key={item.id}><CardContent className="flex flex-wrap items-start justify-between gap-3 pt-6"><div className="max-w-3xl"><p className="font-medium">{item.promptFr}</p><p className="mt-1 text-xs text-muted-foreground">{item.nodeLabel} · {item.responseType} · {item.validatorType} · difficulté {item.difficulty ?? "—"}</p></div><div className="flex flex-wrap gap-2"><Badge variant={item.reviewStatus.includes("approved") ? "success" : "secondary"}>{item.reviewStatus}</Badge>{item.psychometricFlags.length > 0 && <Badge>{item.psychometricFlags.length} signal(s)</Badge>}</div></CardContent></Card>)}</div>
  </>;
}
