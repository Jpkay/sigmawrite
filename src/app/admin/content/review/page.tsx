...
export default async function ReviewPage() {
  ...
  if (session.role === "content_reviewer") await requireActiveReviewer();
  ...
  const visibleCandidates = session.role === "platform_admin"
    ? candidates
    : candidates.filter(candidate => versions.data?.some(version => version.candidate_id === candidate.id));
  ...
  const revision = visibleCandidates.map(candidate => candidate.id + ":" + candidate.updatedAt + ":" + candidate.reviewStatus).join("|") + JSON.stringify(nextSteps);
  return <ReviewClient key={revision} initialCandidates={visibleCandidates} nextSteps={nextSteps} isAdmin={session.role === "platform_admin"} />;
}
