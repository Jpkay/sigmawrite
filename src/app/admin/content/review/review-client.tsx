...
export type ReviewNextStep = { canPublish: boolean; reviewManaged: boolean; href: string; label: string; explanation: string };

export function ReviewClient({ initialCandidates, nextSteps, isAdmin }: { initialCandidates: PersistedCandidate[]; nextSteps: Record<string, ReviewNextStep>; isAdmin: boolean }) {
  ...
  return (
    <>
      ...
      {isAdmin && <details className="group mb-8 border-b border-border pb-5">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div>
          {selected ? (
            <CandidateDetail key={`${selected.id}:${selected.updatedAt}`} candidate={selected} busy={busy !== null} nextStep={nextSteps[selected.id]} isAdmin={isAdmin} error={error} notice={notice}
...
function CandidateDetail({ candidate, busy, nextStep, isAdmin, error, notice, onApprove, onReject, onModerate, onSaveBody }: {
  nextStep?: ReviewNextStep; error: string; notice: string;
  candidate: PersistedCandidate; busy: boolean; isAdmin: boolean; onApprove: () => void; onReject: () => void;
  ...
  const canManageCandidate = isAdmin && !nextStep?.reviewManaged;
  return (
    <section className="border-t-2 border-primary pt-5">
      ...
      <div className="sticky bottom-0 mt-6 space-y-3 border-t border-border bg-background/95 py-4 backdrop-blur">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {!decided && canManageCandidate && <Button variant="destructive" onClick={onReject} disabled={busy || editing}><X /> Rejeter</Button>}
        </div>
      </div>
    </section>
  );
