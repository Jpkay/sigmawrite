...
export default async function ReviewAssignmentAdminPage(){
  ...
  const assignable=versions.filter(v=>["ready_for_review","in_review","review_complete","approved","published"].includes(v.workflowStatus)).sort((a,b)=>new Date(b.candidate.createdAt).getTime()-new Date(a.candidate.createdAt).getTime());
  return <><PageHeader title="Attribuer les passages" description="Sélectionnez les passages et un à trois évaluateurs. Un avis favorable suffit pour la décision éditoriale ; les avis supplémentaires continuent en parallèle."/><ReviewAdminNav/><AssignmentManager currentAdminId={admin.id} versions={assignable.map(v=>({id:v.id,title:v.candidate.generated.title,topic:v.candidate.input.topic,band:v.candidate.input.targetReadingBand,interest:v.candidate.input.primaryInterest,textType:v.candidate.input.textType,assigned:v.assignments.map(a=>a.reviewerProfileId)}))} reviewers={(reviewers??[]) as unknown as Array<{profile_id:string;active:boolean;profiles:{display_name:string|null}}>}/></>;
}
