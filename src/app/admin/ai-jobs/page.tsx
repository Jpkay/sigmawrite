...
import { aiJobTypeLabel, booleanLabel, qualityGateLabel, statusLabel } from "@/lib/presentation/french-labels";
    ...
    {jobs.length === 0 ? <p className="text-sm text-muted-foreground">Aucune tâche enregistrée.</p> : <div className="space-y-3">
      {jobs.map((job) => <Card key={job.id}><CardContent className="space-y-3 pt-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><h2 className="font-medium">{aiJobTypeLabel(job.jobType)}</h2><p className="text-xs text-muted-foreground">{new Date(job.createdAt).toLocaleString("fr-FR")} · {job.provider ?? "—"} / {job.modelId ?? "—"}</p></div>
        ...
        {Object.keys(job.gateOutcomes).length > 0 && <div className="flex flex-wrap gap-2">{Object.entries(job.gateOutcomes).map(([key, value]) => <Badge key={key} variant={value === true ? "success" : value === false ? "outline" : "secondary"}>{qualityGateLabel(key)} : {booleanLabel(value)}</Badge>)}</div>}
