...
export async function GET(_request: Request, { params }: { params: Promise<{ classId: string }> }) {
  ...
  await createServiceClient().from("teacher_reports").insert({ class_id: classId, report_period_start: new Date(Date.now()-7*86_400_000).toISOString().slice(0,10), report_period_end: new Date().toISOString().slice(0,10), report_payload: reportPayload });
