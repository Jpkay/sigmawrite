...
export function PracticePlayer({ practice }: { practice: Practice }) {
  ...
  async function submit() { if (!item) return; setBusy(true); setError(""); try { const result = await submitNodePractice({ nodeId: practice.node.id, itemId: item.id, selectedChoiceId: choice ?? undefined, answerText: choice ? undefined : answer, startedAt: new Date().toISOString(), hintsUsed: hintsShown }); setFeedback({ correct: result.correct, text: result.feedbackFr, mastery: result.mastery }); setRemediation(result.remediation ?? null); setScaffoldLevel(result.scaffoldLevel); if (result.mastered || index + 1 >= Math.min(8, practice.items.length)) setDone(true); } catch { setError("La réponse n’a pas pu être enregistrée."); } finally { setBusy(false); } }
  function next() { setIndex((value) => value + 1); setChoice(null); setAnswer(""); setFeedback(null); setHintsShown(Math.min(2,scaffoldLevel)); setRemediation(null); }
