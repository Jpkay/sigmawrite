...
export default function StudentHome() {
  ...
  return (
    <>
      ...
      {plan.length > 0 && <section className="mb-10"><div className="mb-4 flex items-end justify-between"><div><p className="font-display text-xs font-semibold uppercase tracking-[0.16em] text-success">Parcours personnalisé</p><h2 className="mt-1 text-xl font-semibold">Les prochaines étapes</h2></div><span className="text-sm text-muted-foreground">{plan.length} à faire</span></div><div className="border-y border-border">{plan.map((step, index) => <div key={step.nodeId} className="group grid gap-4 border-b border-border py-4 last:border-0 sm:grid-cols-[3rem_1fr_auto] sm:items-center"><span className="grid size-10 place-items-center rounded-full border-2 border-dashed border-locked-border font-display text-sm font-bold text-locked group-first:border-solid group-first:border-primary group-first:bg-primary group-first:text-primary-foreground">{index + 1}</span><div><p className="font-semibold">{step.label}</p><div className="mt-2 h-1.5 max-w-sm overflow-hidden rounded-full bg-rail"><div className="h-full rounded-full bg-success" style={{width:`${Math.round(step.mastery*100)}%`}} /></div><p className="mt-1.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">Maîtrise {Math.round(step.mastery * 100)}%</p></div><Link href={`/student/practice/${step.nodeId}`} className={buttonVariants({ variant: index === 0 ? "default" : "outline", size: "sm" })}>S’entraîner <ArrowRight /></Link></div>)}</div></section>}
