...
export default function StudentHome() {
  ...
  return (
    <>
      ...
      <div className="mb-6 flex flex-wrap gap-3"><Link href="/student/lessons" className={buttonVariants()}>Mes leçons <ArrowRight /></Link><Link href="/student/diagnostic" className={buttonVariants({variant:"outline"})}>Voir mes résultats</Link></div>

      {assessment?.required && assessment.kind === "reentry" && assessment.reason === "inactivity" && <Card className="mb-6 border-primary/40 bg-accent/40"><CardContent className="flex flex-wrap items-center justify-between gap-4 pt-6"><div><p className="font-medium">Mets ton profil à jour</p><p className="text-sm text-muted-foreground">Quelques questions ciblées suffisent. Tes progrès précédents sont conservés.</p></div><Link href="/student/diagnostic?restart=1" className={buttonVariants()}>Mettre à jour <ArrowRight /></Link></CardContent></Card>}
