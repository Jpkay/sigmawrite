...
import {
  DICTATION_COPY,
  dictationBlankLabel,
  dictationCorrectionHeading,
  dictationHeaderEyebrow,
  dictationIntro,
  dictationJustificationOutcomeDisplay,
  dictationPlayerError,
  dictationReplayCount,
  dictationRuleLabel,
  dictationSegmentLabel,
  dictationSegmentPosition,
...
export function DictationPlayer({ dictationId }: { dictationId: string }) {
  ...
  useEffect(() => {
    ...
    }).catch((caught) => { if (active) { setError(dictationPlayerError("start", caught)); setPhase("intro"); } });
    return () => { active = false; };
  ...
  if (phase === "loading") return <p className="text-sm text-muted-foreground">{DICTATION_COPY.preparing}</p>;
  if (!session) return <div><p role="alert" className="text-sm text-destructive">{error || DICTATION_COPY.unavailable}</p><Link href="/student/dictee" className={`${buttonVariants({ variant: "outline" })} mt-4`}><ArrowLeft className="size-4" />{DICTATION_COPY.allDictations}</Link></div>;
  ...
  async function submit() {
    ...
    } catch (caught) { setError(dictationPlayerError("score", caught)); setPhase("write"); }
  ...
  async function sendJustifications() {
    ...
    } catch (caught) { setError(dictationPlayerError("justify", caught)); }
    finally { setBusy(false); }
  }

  const header = (
    <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
      <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">{dictationHeaderEyebrow(session.mode)}</p><h1 className="mt-1 font-display text-2xl font-semibold">{session.title}</h1></div>
      <Link href="/student/dictee" className="text-sm text-muted-foreground underline-offset-4 hover:underline">{DICTATION_COPY.quit}</Link>
    </div>
  );
    ...
    return (
      <div className="mx-auto max-w-2xl">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          <p className="text-lg leading-8">{dictationIntro(session, total)}</p>
          <ol className="mt-4 list-decimal space-y-1 pl-5 text-sm text-muted-foreground">
            {DICTATION_COPY.introSteps.map((step) => <li key={step}>{step}</li>)}
          </ol>
          {session.audioMode === "browser" && <p className="mt-4 rounded-md bg-amber-500/10 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">{DICTATION_COPY.browserVoice}</p>}
          <div className="mt-6 flex flex-wrap gap-3">
            <Button variant="outline" onClick={() => play(session.fullAudioUrl, session.audioMode === "browser" ? session.segments.map((entry) => entry.browserText ?? "").join(" ") : null)}><Headphones className="size-4" />{DICTATION_COPY.listenAll}</Button>
            <Button onClick={() => { setPhase("write"); play(current.audioUrl, current.browserText); }}>{DICTATION_COPY.beginWriting} <ArrowRight /></Button>
          </div>
        </div>
      </div>
    );
  ...
  if (phase === "write" || phase === "scoring") {
    return (
      <div className="mx-auto max-w-2xl pb-28 sm:pb-0">
        {header}
        <div className="mb-4 flex items-center justify-between text-sm text-muted-foreground"><span>{dictationSegmentPosition(segment, total)}</span><span aria-live="polite">{dictationReplayCount(replays)}</span></div>
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          <div className="flex flex-wrap gap-2">
            <Button type="button" variant="outline" onClick={() => { setReplays((n) => n + 1); play(current.audioUrl, current.browserText); }}><Play className="size-4" />{DICTATION_COPY.replaySegment}</Button>
          </div>
          {withTemplate ? (
            <p className="mt-5 flex flex-wrap items-baseline gap-x-1.5 gap-y-3 text-lg leading-9">
              {current.template!.tokens.map((token, position) => {
                ...
                if (blank.choices) return (
                  <span key={position} role="radiogroup" aria-label={dictationBlankLabel(position)} className="inline-flex gap-1 rounded-md border border-dashed border-primary/60 p-1">
        ...
        <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-background/95 p-3 pb-[max(.75rem,env(safe-area-inset-bottom))] backdrop-blur-xl sm:static sm:mt-5 sm:border-0 sm:bg-transparent sm:p-0">
          <div className="mx-auto flex max-w-2xl items-center justify-between gap-3">
              <Button type="button" variant="ghost" disabled={segment === 0} onClick={() => setSegment((n) => n - 1)}><ArrowLeft className="size-4" />{DICTATION_COPY.previous}</Button>
            {segment + 1 < total
              ? <Button type="button" onClick={() => { const next = segment + 1; setSegment(next); play(session.segments[next].audioUrl, session.segments[next].browserText); }}>{DICTATION_COPY.nextSegment} <ArrowRight /></Button>
              : <Button type="button" disabled={!canSubmit || busy} onClick={() => void submit()}>{busy ? DICTATION_COPY.correcting : DICTATION_COPY.validate} <CheckCircle2 className="size-4" /></Button>}
          </div>
        </div>
      </div>
    );
  ...
  if (phase === "justify" && result) {
    ...
    return (
      <div className="mx-auto max-w-2xl">
        {header}
        <div className="rounded-xl border border-border bg-card p-6">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-primary">{DICTATION_COPY.negotiation}</p>
          <h2 className="mt-2 text-xl font-semibold">{dictationCorrectionHeading(allErrors.length)}</h2>
          <p className="mt-1 text-sm text-muted-foreground">{DICTATION_COPY.negotiationHelp}</p>
            ...
            {result.justification.map((entry) => {
              ...
              return (
                <li key={entry.errorIndex} className="border-t border-border pt-5">
                  <p className="text-sm text-muted-foreground">{dictationSegmentLabel(item.segment)} · tu as écrit <span className="font-semibold text-foreground">« {item.actual ?? DICTATION_COPY.missingWord} »</span></p>
                  <div role="radiogroup" aria-label={dictationRuleLabel(entry.errorIndex)} className="mt-3 grid gap-2 sm:grid-cols-2">
          ...
          {error && <p role="alert" className="mt-4 text-sm text-destructive">{error}</p>}
          <div className="mt-6 flex justify-end"><Button disabled={pending > 0 || busy} onClick={() => void sendJustifications()}>{busy ? DICTATION_COPY.saving : DICTATION_COPY.seeCorrection} <ArrowRight /></Button></div>
        </div>
      </div>
    );
  }
  ...
  if (phase === "result" && result) {
    return <DictationResultPanel result={result} justificationOutcome={justificationOutcome} dictationId={dictationId} header={header} />;
  }
  return null;
}

export function DictationResultPanel({ result, justificationOutcome, dictationId, header }: {
  result: DictationResult;
  justificationOutcome: { correct: number; total: number } | null;
  dictationId: string;
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <section className="rounded-xl border border-border bg-card-elevated p-6">
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">{DICTATION_COPY.score}</p><p className="font-display text-5xl font-bold tabular-nums">{result.score}<span className="text-2xl text-muted-foreground"> / 10</span></p></div>
        ...
        {justificationOutcome && <p className="mt-3 text-sm text-muted-foreground">{dictationJustificationOutcomeDisplay(justificationOutcome).text}</p>}
        {categories.length > 0 ? (
        ...
        ) : <p className="mt-4 flex items-center gap-2 text-sm text-success"><CheckCircle2 className="size-4" />{DICTATION_COPY.clean}</p>}
      </section>
      <ol className="mt-6 space-y-4">
        {result.segments.map((entry) => (
          <li key={entry.index} className="rounded-lg border border-border bg-card p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">{dictationSegmentLabel(entry.index)}</p>
            <p className="mt-2 text-lg leading-8">{entry.expected}</p>
            {entry.errors.length === 0 ? <p className="mt-1 flex items-center gap-1.5 text-sm text-success"><CheckCircle2 className="size-4" />{DICTATION_COPY.exact}</p> : (
              <ul className="mt-3 space-y-3">
                {entry.errors.map((item, index) => (
                  <li key={index} className="flex gap-3 border-l-2 border-amber-500 pl-3 text-sm">
                    ...
                    <div>
                      <p><span className="line-through decoration-amber-600">{item.actual ?? DICTATION_COPY.omitted}</span> → <span className="font-semibold">{item.expected || DICTATION_COPY.extra}</span> <span className="ml-1 text-xs text-muted-foreground">{result.categoryLabels[item.category]}</span></p>
                      <p className="mt-1 text-muted-foreground">{item.explanationFr}</p>
                      <Link href={`/student/reference/regle/${encodeURIComponent(item.nodeKey)}`} className="mt-1 inline-block text-xs font-medium text-primary underline-offset-4 hover:underline">{DICTATION_COPY.seeRule}</Link>
                    </div>
                  </li>
                ))}
              </ul>
      ...
      <div className="mt-8 flex flex-wrap gap-3">
        <Link href="/student/dictee" className={buttonVariants({ variant: "outline" })}><ArrowLeft className="size-4" />{DICTATION_COPY.otherDictations}</Link>
        <Link href={`/student/dictee/${dictationId}`} className={buttonVariants({ variant: "outline" })} onClick={() => window.location.reload()}><RotateCcw className="size-4" />{DICTATION_COPY.retry}</Link>
        <Link href="/student" className={buttonVariants()}>{DICTATION_COPY.returnToPlan} <ArrowRight /></Link>
      </div>
    </div>
  );
}
