...
export function ReadingPlayer({textKey, text}: {textKey: string; text: SeedText | null}) {
  ...
  useEffect(() => {
    if (!hasStudentBackend || !text) return;
  ...
  useEffect(() => () => { if (!finished.current) track("reading_session_abandoned", { text_id: textKey, phase: phaseRef.current }); }, [textKey]);
