...
select is((select count(*) from public.competency_items item join public.competency_nodes node on node.id=item.primary_node_id where item.prompt_version='pronoun-practice-v1' and node.key='construction_pronom_objet'),36::bigint,'Every exercise belongs to the approved object-pronoun competency');
