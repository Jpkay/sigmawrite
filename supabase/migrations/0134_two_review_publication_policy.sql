-- Two independent favorable reviews unlock editorial approval; further reviews
-- remain assigned and may be submitted after publication.
alter table public.content_review_settings alter column required_reviewers set default 2;
alter table public.content_review_versions alter column required_reviewers set default 2;
update public.content_review_settings set required_reviewers=2, updated_at=now() where id=true;
...
begin
  ...
  perform 1 from public.content_review_versions where id=p_review_version_id for update;
  if p_action='approve' and (
    select count(distinct pr.reviewer_profile_id) from public.passage_reviews pr
    join public.review_assignments ra on ra.id=pr.assignment_id
    where ra.review_version_id=p_review_version_id and pr.status='submitted'
      and pr.overall_decision in ('approve','approve_minor')
  ) < (select required_reviewers from public.content_review_versions where id=p_review_version_id)
  then raise exception 'independent_approvals_required'; end if;
...
-- Preserve explicit requests for additional evidence and all past review records.
do $$
declare v_id uuid;
begin
  for v_id in
    update public.content_review_versions v set required_reviewers=2, updated_at=now()
    where v.required_reviewers=3
      and v.workflow_status in ('ready_for_review','in_review','review_complete')
      and not exists (
        select 1 from public.editorial_resolutions r
        where r.review_version_id=v.id and r.action='request_another_review'
      )
    returning v.id
  loop
    perform public.refresh_content_review_status(v_id);
  end loop;
end;
$$;

insert into public.audit_logs(action,target_type,metadata)
values ('review.publication_policy_changed','content_review_settings',
  '{"requiredIndependentApprovals":2,"additionalReviewsContinue":true,"decisionDate":"2026-09-09"}'::jsonb);
